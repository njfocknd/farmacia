...
class cusuario_list extends cusuario {
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->contrasena, $Default, FALSE); // contrasena
		...
		if (!$Default && $this->Command == "search") {
			...
			$this->contrasena->AdvancedSearch->Save(); // contrasena
	...
	function CheckSearchParms() {
		...
		if ($this->usuario->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->contrasena->AdvancedSearch->IssetSession())
			return TRUE;
	...
	function ResetAdvancedSearchParms() {
		...
		$this->contrasena->AdvancedSearch->UnsetSession();
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->idusuario->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'>";
	...
	function SetupSearchOptions() {
		...
		if (ew_IsMobile())
			$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"usuariosrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		else
			$item->Body = "<button type=\"button\" class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" onclick=\"ew_SearchDialogShow({lnk:this,url:'usuariosrch.php'});\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;
	...
	function LoadSearchValues() {
		...
		// contrasena
		$this->contrasena->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_contrasena"]);
		if ($this->contrasena->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->contrasena->AdvancedSearch->SearchOperator = @$_GET["z_contrasena"];
...
fusuariolist.Lists["x_userlevelid"] = {"LinkField":"x_userlevelid","Ajax":null,"AutoFill":false,"DisplayFields":["x_userlevelname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php
	...
	if ($usuario->CurrentAction == "" && $usuario_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$usuario_list->setWarningMessage($Language->Phrase("NoPermission"));
...
<?php if ($usuario->idusuario->Visible) { // idusuario ?>
	<?php if ($usuario->SortUrl($usuario->idusuario) == "") { ?>
		<th data-name="idusuario"><div id="elh_usuario_idusuario" class="usuario_idusuario"><div class="ewTableHeaderCaption"><?php echo $usuario->idusuario->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="idusuario"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $usuario->SortUrl($usuario->idusuario) ?>',1);"><div id="elh_usuario_idusuario" class="usuario_idusuario">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $usuario->idusuario->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($usuario->idusuario->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($usuario->idusuario->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
...
<?php if ($usuario->estado->Visible) { // estado ?>
	<?php if ($usuario->SortUrl($usuario->estado) == "") { ?>
		<th data-name="estado"><div id="elh_usuario_estado" class="usuario_estado"><div class="ewTableHeaderCaption"><?php echo $usuario->estado->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="estado"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $usuario->SortUrl($usuario->estado) ?>',1);"><div id="elh_usuario_estado" class="usuario_estado">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $usuario->estado->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($usuario->estado->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($usuario->estado->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<?php echo $usuario->estado->ListViewValue() ?></span>
</td>
	<?php } ?>
<?php
