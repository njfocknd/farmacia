...
function ew_ScrollableTable(elContainer, width, height) {
	...
	$(function() {
		...
		// check last row
		if (yscroll && tbd.rows && bdContainer.offsetHeight > tbd.offsetHeight) {
			...
			$rows.filter(":gt(" + (-1 * n - 1) + ")").find("td." + EW_TABLE_LAST_ROW_CLASSNAME).removeClass(EW_TABLE_LAST_ROW_CLASSNAME);
		}
	});	
}
