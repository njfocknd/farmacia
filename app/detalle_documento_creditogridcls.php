...
$detalle_documento_credito_grid = NULL; // Initialize page object first

class cdetalle_documento_credito_grid extends cdetalle_documento_credito {
	...
	// Page object name
	var $PageObjName = 'detalle_documento_credito_grid';

	// Grid form hidden field names
	var $FormName = 'fdetalle_documento_creditogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detalle_documento_credito"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('iddetalle_documento_credito');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddetalle_documento_credito->CurrentValue = strval($arKeys[0]); // iddetalle_documento_credito
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "documento_credito") {
			$this->iddocumento_credito->Visible = FALSE;
			if ($GLOBALS["documento_credito"]->EventCancelled) $this->EventCancelled = TRUE;
		}
