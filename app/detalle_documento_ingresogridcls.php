...
$detalle_documento_ingreso_grid = NULL; // Initialize page object first

class cdetalle_documento_ingreso_grid extends cdetalle_documento_ingreso {
	...
	// Page object name
	var $PageObjName = 'detalle_documento_ingreso_grid';

	// Grid form hidden field names
	var $FormName = 'fdetalle_documento_ingresogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detalle_documento_ingreso"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->iddetalle_documento_ingreso->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_iddocumento_ingreso") && $objForm->HasValue("o_iddocumento_ingreso") && $this->iddocumento_ingreso->CurrentValue <> $this->iddocumento_ingreso->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->iddetalle_documento_ingreso->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('iddetalle_documento_ingreso');
	}
	...
	function LoadFormValues() {
		...
		$this->iddocumento_ingreso->setOldValue($objForm->GetValue("o_iddocumento_ingreso"));
		...
		if (!$this->iddetalle_documento_ingreso->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddetalle_documento_ingreso->CurrentValue = strval($arKeys[0]); // iddetalle_documento_ingreso
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['iddetalle_documento_ingreso'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// iddocumento_ingreso
			$this->iddocumento_ingreso->SetDbValueDef($rsnew, $this->iddocumento_ingreso->CurrentValue, 0, $this->iddocumento_ingreso->ReadOnly);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "documento_ingreso") {
			$this->iddocumento_ingreso->Visible = FALSE;
			if ($GLOBALS["documento_ingreso"]->EventCancelled) $this->EventCancelled = TRUE;
		}
