...
$documento_movimiento = NULL;

//
// Table class for documento_movimiento
//
class cdocumento_movimiento extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'documento_movimiento';
		$this->TableName = 'documento_movimiento';
		...
		$this->iddocumento_movimiento = new cField('documento_movimiento', 'documento_movimiento', 'x_iddocumento_movimiento', 'iddocumento_movimiento', '`iddocumento_movimiento`', '`iddocumento_movimiento`', 3, -1, FALSE, '`iddocumento_movimiento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// idtipo_documento
		$this->idtipo_documento = new cField('documento_movimiento', 'documento_movimiento', 'x_idtipo_documento', 'idtipo_documento', '`idtipo_documento`', '`idtipo_documento`', 3, -1, FALSE, '`idtipo_documento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// idserie_documento
		$this->idserie_documento = new cField('documento_movimiento', 'documento_movimiento', 'x_idserie_documento', 'idserie_documento', '`idserie_documento`', '`idserie_documento`', 3, -1, FALSE, '`idserie_documento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// serie
		$this->serie = new cField('documento_movimiento', 'documento_movimiento', 'x_serie', 'serie', '`serie`', '`serie`', 200, -1, FALSE, '`serie`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['serie'] = &$this->serie;

		// correlativo
		$this->correlativo = new cField('documento_movimiento', 'documento_movimiento', 'x_correlativo', 'correlativo', '`correlativo`', '`correlativo`', 3, -1, FALSE, '`correlativo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['correlativo'] = &$this->correlativo;

		// fecha
		$this->fecha = new cField('documento_movimiento', 'documento_movimiento', 'x_fecha', 'fecha', '`fecha`', 'DATE_FORMAT(`fecha`, \'%d/%m/%Y\')', 133, 7, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha'] = &$this->fecha;

		// observaciones
		$this->observaciones = new cField('documento_movimiento', 'documento_movimiento', 'x_observaciones', 'observaciones', '`observaciones`', '`observaciones`', 200, -1, FALSE, '`observaciones`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['observaciones'] = &$this->observaciones;

		// estado_documento
		$this->estado_documento = new cField('documento_movimiento', 'documento_movimiento', 'x_estado_documento', 'estado_documento', '`estado_documento`', '`estado_documento`', 202, -1, FALSE, '`estado_documento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['estado_documento'] = &$this->estado_documento;

		// idsucursal_ingreso
		$this->idsucursal_ingreso = new cField('documento_movimiento', 'documento_movimiento', 'x_idsucursal_ingreso', 'idsucursal_ingreso', '`idsucursal_ingreso`', '`idsucursal_ingreso`', 3, -1, FALSE, '`idsucursal_ingreso`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// idsucursal_egreso
		$this->idsucursal_egreso = new cField('documento_movimiento', 'documento_movimiento', 'x_idsucursal_egreso', 'idsucursal_egreso', '`idsucursal_egreso`', '`idsucursal_egreso`', 3, -1, FALSE, '`idsucursal_egreso`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// estado
		$this->estado = new cField('documento_movimiento', 'documento_movimiento', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['estado'] = &$this->estado;

		// fecha_insercion
		$this->fecha_insercion = new cField('documento_movimiento', 'documento_movimiento', 'x_fecha_insercion', 'fecha_insercion', '`fecha_insercion`', 'DATE_FORMAT(`fecha_insercion`, \'%d/%m/%Y\')', 135, 7, FALSE, '`fecha_insercion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// monto
		$this->monto = new cField('documento_movimiento', 'documento_movimiento', 'x_monto', 'monto', '`monto`', '`monto`', 131, -1, FALSE, '`monto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`documento_movimiento`";
	}
	...
	// Update Table
	var $UpdateTable = "`documento_movimiento`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('iddocumento_movimiento', $rs))
				ew_AddFilter($where, ew_QuotedName('iddocumento_movimiento') . '=' . ew_QuotedValue($rs['iddocumento_movimiento'], $this->iddocumento_movimiento->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`iddocumento_movimiento` = @iddocumento_movimiento@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->iddocumento_movimiento->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@iddocumento_movimiento@", ew_AdjustSql($this->iddocumento_movimiento->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "documento_movimientolist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("documento_movimientoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("documento_movimientoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "documento_movimientoadd.php?" . $this->UrlParm($parm);
		else
			return "documento_movimientoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("documento_movimientoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("documento_movimientoadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("documento_movimientodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->iddocumento_movimiento->CurrentValue)) {
			$sUrl .= "iddocumento_movimiento=" . urlencode($this->iddocumento_movimiento->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["iddocumento_movimiento"]; // iddocumento_movimiento
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->iddocumento_movimiento->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->iddocumento_movimiento->EditValue = $this->iddocumento_movimiento->CurrentValue;
		$this->iddocumento_movimiento->ViewCustomAttributes = "";
