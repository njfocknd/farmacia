...
$documento_interno_add = NULL; // Initialize page object first

class cdocumento_interno_add extends cdocumento_interno {
	...
	// Page object name
	var $PageObjName = 'documento_interno_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["iddocumento_interno"] != "") {
				...
				$this->setKey("iddocumento_interno", $this->iddocumento_interno->CurrentValue); // Set up key
			} else {
				$this->setKey("iddocumento_interno", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("documento_internolist.php"); // No matching record, return to list
				}
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "documento_internoview.php")
	...
	function ValidateForm() {
		...
		if (in_array("detalle_documento_interno", $DetailTblVar) && $GLOBALS["detalle_documento_interno"]->DetailAdd) {
			if (!isset($GLOBALS["detalle_documento_interno_grid"])) $GLOBALS["detalle_documento_interno_grid"] = new cdetalle_documento_interno_grid(); // get detail page object
			$GLOBALS["detalle_documento_interno_grid"]->ValidateGridForm();
		}
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->iddocumento_interno->setDbValue($conn->Insert_ID());
			$rsnew['iddocumento_interno'] = $this->iddocumento_interno->DbValue;
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("detalle_documento_interno", $DetailTblVar) && $GLOBALS["detalle_documento_interno"]->DetailAdd) {
				$GLOBALS["detalle_documento_interno"]->iddocumento_interno->setSessionValue($this->iddocumento_interno->CurrentValue); // Set master key
				if (!isset($GLOBALS["detalle_documento_interno_grid"])) $GLOBALS["detalle_documento_interno_grid"] = new cdetalle_documento_interno_grid(); // Get detail page object
				$AddRow = $GLOBALS["detalle_documento_interno_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["detalle_documento_interno"]->iddocumento_interno->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("detalle_documento_interno", $DetailTblVar)) {
				...
				if ($GLOBALS["detalle_documento_interno_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["detalle_documento_interno_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["detalle_documento_interno_grid"]->CurrentMode = "add";
					$GLOBALS["detalle_documento_interno_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($documento_interno_add)) $documento_interno_add = new cdocumento_interno_add();

// Page init
$documento_interno_add->Page_Init();

// Page main
$documento_interno_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$documento_interno_add->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var documento_interno_add = new ew_Page("documento_interno_add");
documento_interno_add.PageID = "add"; // Page ID
var EW_PAGE_ID = documento_interno_add.PageID; // For backward compatibility

// Form object
var fdocumento_internoadd = new ew_Form("fdocumento_internoadd");

// Validate form
fdocumento_internoadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $documento_interno->idtipo_documento->FldCaption(), $documento_interno->idtipo_documento->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_idserie_documento");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $documento_interno->idserie_documento->FldCaption(), $documento_interno->idserie_documento->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_fecha");
			if (elm && !ew_CheckEuroDate(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($documento_interno->fecha->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_idsucursal_ingreso");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $documento_interno->idsucursal_ingreso->FldCaption(), $documento_interno->idsucursal_ingreso->ReqErrMsg)) ?>");
...
fdocumento_internoadd.Form_CustomValidate = 
...
fdocumento_internoadd.ValidateRequired = true;
<?php } else { ?>
fdocumento_internoadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdocumento_internoadd.Lists["x_idtipo_documento"] = {"LinkField":"x_idtipo_documento","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fdocumento_internoadd.Lists["x_idserie_documento"] = {"LinkField":"x_idserie_documento","Ajax":true,"AutoFill":false,"DisplayFields":["x_serie","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fdocumento_internoadd.Lists["x_idsucursal_ingreso"] = {"LinkField":"x_idsucursal","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fdocumento_internoadd.Lists["x_idsucursal_egreso"] = {"LinkField":"x_idsucursal","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $documento_interno_add->ShowPageHeader(); ?>
<?php
$documento_interno_add->ShowMessage();
?>
<form name="fdocumento_internoadd" id="fdocumento_internoadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($documento_interno_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $documento_interno_add->Token ?>">
<?php } ?>
...
<?php if ($documento_interno->idtipo_documento->Visible) { // idtipo_documento ?>
	<div id="r_idtipo_documento" class="form-group">
		<label id="elh_documento_interno_idtipo_documento" for="x_idtipo_documento" class="col-sm-2 control-label ewLabel"><?php echo $documento_interno->idtipo_documento->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $documento_interno->idtipo_documento->CellAttributes() ?>>
<span id="el_documento_interno_idtipo_documento">
<select data-field="x_idtipo_documento" id="x_idtipo_documento" name="x_idtipo_documento"<?php echo $documento_interno->idtipo_documento->EditAttributes() ?>>
<?php
if (is_array($documento_interno->idtipo_documento->EditValue)) {
	$arwrk = $documento_interno->idtipo_documento->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($documento_interno->idtipo_documento->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
// Call Lookup selecting
$documento_interno->Lookup_Selecting($documento_interno->idtipo_documento, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
...
<?php echo $documento_interno->idtipo_documento->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($documento_interno->idserie_documento->Visible) { // idserie_documento ?>
	<div id="r_idserie_documento" class="form-group">
		<label id="elh_documento_interno_idserie_documento" for="x_idserie_documento" class="col-sm-2 control-label ewLabel"><?php echo $documento_interno->idserie_documento->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $documento_interno->idserie_documento->CellAttributes() ?>>
<span id="el_documento_interno_idserie_documento">
<select data-field="x_idserie_documento" id="x_idserie_documento" name="x_idserie_documento"<?php echo $documento_interno->idserie_documento->EditAttributes() ?>>
<?php
if (is_array($documento_interno->idserie_documento->EditValue)) {
	$arwrk = $documento_interno->idserie_documento->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($documento_interno->idserie_documento->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
// Call Lookup selecting
$documento_interno->Lookup_Selecting($documento_interno->idserie_documento, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
...
<?php echo $documento_interno->idserie_documento->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($documento_interno->fecha->Visible) { // fecha ?>
	<div id="r_fecha" class="form-group">
		<label id="elh_documento_interno_fecha" for="x_fecha" class="col-sm-2 control-label ewLabel"><?php echo $documento_interno->fecha->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $documento_interno->fecha->CellAttributes() ?>>
<span id="el_documento_interno_fecha">
<input type="text" data-field="x_fecha" name="x_fecha" id="x_fecha" placeholder="<?php echo ew_HtmlEncode($documento_interno->fecha->PlaceHolder) ?>" value="<?php echo $documento_interno->fecha->EditValue ?>"<?php echo $documento_interno->fecha->EditAttributes() ?>>
<?php if (!$documento_interno->fecha->ReadOnly && !$documento_interno->fecha->Disabled && @$documento_interno->fecha->EditAttrs["readonly"] == "" && @$documento_interno->fecha->EditAttrs["disabled"] == "") { ?>
<script type="text/javascript">
ew_CreateCalendar("fdocumento_internoadd", "x_fecha", "%d/%m/%Y");
</script>
<?php } ?>
</span>
<?php echo $documento_interno->fecha->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($documento_interno->observaciones->Visible) { // observaciones ?>
	<div id="r_observaciones" class="form-group">
		<label id="elh_documento_interno_observaciones" for="x_observaciones" class="col-sm-2 control-label ewLabel"><?php echo $documento_interno->observaciones->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $documento_interno->observaciones->CellAttributes() ?>>
<span id="el_documento_interno_observaciones">
<input type="text" data-field="x_observaciones" name="x_observaciones" id="x_observaciones" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($documento_interno->observaciones->PlaceHolder) ?>" value="<?php echo $documento_interno->observaciones->EditValue ?>"<?php echo $documento_interno->observaciones->EditAttributes() ?>>
</span>
<?php echo $documento_interno->observaciones->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($documento_interno->idsucursal_ingreso->Visible) { // idsucursal_ingreso ?>
	<div id="r_idsucursal_ingreso" class="form-group">
		<label id="elh_documento_interno_idsucursal_ingreso" for="x_idsucursal_ingreso" class="col-sm-2 control-label ewLabel"><?php echo $documento_interno->idsucursal_ingreso->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $documento_interno->idsucursal_ingreso->CellAttributes() ?>>
<span id="el_documento_interno_idsucursal_ingreso">
<select data-field="x_idsucursal_ingreso" id="x_idsucursal_ingreso" name="x_idsucursal_ingreso"<?php echo $documento_interno->idsucursal_ingreso->EditAttributes() ?>>
<?php
if (is_array($documento_interno->idsucursal_ingreso->EditValue)) {
	$arwrk = $documento_interno->idsucursal_ingreso->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($documento_interno->idsucursal_ingreso->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
// Call Lookup selecting
$documento_interno->Lookup_Selecting($documento_interno->idsucursal_ingreso, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
...
<?php echo $documento_interno->idsucursal_ingreso->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($documento_interno->idsucursal_egreso->Visible) { // idsucursal_egreso ?>
	<div id="r_idsucursal_egreso" class="form-group">
		<label id="elh_documento_interno_idsucursal_egreso" for="x_idsucursal_egreso" class="col-sm-2 control-label ewLabel"><?php echo $documento_interno->idsucursal_egreso->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $documento_interno->idsucursal_egreso->CellAttributes() ?>>
<span id="el_documento_interno_idsucursal_egreso">
<select data-field="x_idsucursal_egreso" id="x_idsucursal_egreso" name="x_idsucursal_egreso"<?php echo $documento_interno->idsucursal_egreso->EditAttributes() ?>>
<?php
if (is_array($documento_interno->idsucursal_egreso->EditValue)) {
	$arwrk = $documento_interno->idsucursal_egreso->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($documento_interno->idsucursal_egreso->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
// Call Lookup selecting
$documento_interno->Lookup_Selecting($documento_interno->idsucursal_egreso, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
$sSqlWrk .= " ORDER BY `nombre`";
?>
...
<?php
	if (in_array("detalle_documento_interno", explode(",", $documento_interno->getCurrentDetailTable())) && $detalle_documento_interno->DetailAdd) {
?>
...
<script type="text/javascript">
fdocumento_internoadd.Init();
</script>
<?php
$documento_interno_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$documento_interno_add->Page_Terminate();
?>
