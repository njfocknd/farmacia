...
//
// Table class for medicamento
//
class cmedicamento extends cTable {
	var $idmedicamento;
	var $nombre;
	var $idpais;
	var $idmarca;
	...
	function __construct() {
		...
		// idmedicamento
		$this->idmedicamento = new cField('medicamento', 'medicamento', 'x_idmedicamento', 'idmedicamento', '`idmedicamento`', '`idmedicamento`', 3, -1, FALSE, '`idmedicamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idmedicamento->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idmedicamento'] = &$this->idmedicamento;

		// nombre
		$this->nombre = new cField('medicamento', 'medicamento', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nombre'] = &$this->nombre;

		// idpais
		$this->idpais = new cField('medicamento', 'medicamento', 'x_idpais', 'idpais', '`idpais`', '`idpais`', 3, -1, FALSE, '`idpais`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idpais->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idpais'] = &$this->idpais;

		// idmarca
		$this->idmarca = new cField('medicamento', 'medicamento', 'x_idmarca', 'idmarca', '`idmarca`', '`idmarca`', 3, -1, FALSE, '`idmarca`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idmarca->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idmarca'] = &$this->idmarca;

		// estado
		$this->estado = new cField('medicamento', 'medicamento', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "registro_sanitario") {
			...
			$sDetailUrl .= "&fk_idmedicamento=" . urlencode($this->idmedicamento->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "medicamentolist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`medicamento`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('idmedicamento', $rs))
				ew_AddFilter($where, ew_QuotedName('idmedicamento') . '=' . ew_QuotedValue($rs['idmedicamento'], $this->idmedicamento->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->idmedicamento->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idmedicamento@", ew_AdjustSql($this->idmedicamento->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("medicamentoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("medicamentoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "medicamentoadd.php?" . $this->UrlParm($parm);
		else
			return "medicamentoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("medicamentoedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("medicamentoedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("medicamentoadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("medicamentoadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("medicamentodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idmedicamento->CurrentValue)) {
			$sUrl .= "idmedicamento=" . urlencode($this->idmedicamento->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["idmedicamento"]; // idmedicamento
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->idmedicamento->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->idmedicamento->EditValue = $this->idmedicamento->CurrentValue;
		$this->idmedicamento->ViewCustomAttributes = "";
