...
$boleta_deposito_edit = NULL; // Initialize page object first

class cboleta_deposito_edit extends cboleta_deposito {
	...
	// Page object name
	var $PageObjName = 'boleta_deposito_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idboleta_deposito->CurrentValue == "")
			$this->Page_Terminate("boleta_depositolist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// efectivo
			$this->efectivo->SetDbValueDef($rsnew, $this->efectivo->CurrentValue, 0, $this->efectivo->ReadOnly);

			// cheque
			$this->cheque->SetDbValueDef($rsnew, $this->cheque->CurrentValue, 0, $this->cheque->ReadOnly);

			// cheque_otro
			$this->cheque_otro->SetDbValueDef($rsnew, $this->cheque_otro->CurrentValue, 0, $this->cheque_otro->ReadOnly);
...
// Create page object
if (!isset($boleta_deposito_edit)) $boleta_deposito_edit = new cboleta_deposito_edit();

// Page init
$boleta_deposito_edit->Page_Init();

// Page main
$boleta_deposito_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$boleta_deposito_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var boleta_deposito_edit = new ew_Page("boleta_deposito_edit");
boleta_deposito_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = boleta_deposito_edit.PageID; // For backward compatibility

// Form object
var fboleta_depositoedit = new ew_Form("fboleta_depositoedit");

// Validate form
fboleta_depositoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $boleta_deposito->estado->FldCaption(), $boleta_deposito->estado->ReqErrMsg)) ?>");
...
fboleta_depositoedit.Form_CustomValidate = 
...
fboleta_depositoedit.ValidateRequired = true;
<?php } else { ?>
fboleta_depositoedit.ValidateRequired = false; 
<?php } ?>
...
<?php $boleta_deposito_edit->ShowPageHeader(); ?>
<?php
$boleta_deposito_edit->ShowMessage();
?>
<form name="fboleta_depositoedit" id="fboleta_depositoedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($boleta_deposito_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $boleta_deposito_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fboleta_depositoedit", "x_fecha", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<?php if ($boleta_deposito->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_boleta_deposito_estado" class="col-sm-2 control-label ewLabel"><?php echo $boleta_deposito->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $boleta_deposito->estado->CellAttributes() ?>>
<span id="el_boleta_deposito_estado">
<div id="tp_x_estado" class="<?php echo EW_ITEM_TEMPLATE_CLASSNAME ?>"><input type="radio" name="x_estado" id="x_estado" value="{value}"<?php echo $boleta_deposito->estado->EditAttributes() ?>></div>
<div id="dsl_x_estado" data-repeatcolumn="5" class="ewItemList">
<?php
$arwrk = $boleta_deposito->estado->EditValue;
if (is_array($arwrk)) {
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($boleta_deposito->estado->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;

		// Note: No spacing within the LABEL tag
?>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 1) ?>
<label class="radio-inline"><input type="radio" data-field="x_estado" name="x_estado" id="x_estado_<?php echo $rowcntwrk ?>" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $boleta_deposito->estado->EditAttributes() ?>><?php echo $arwrk[$rowcntwrk][1] ?></label>
...
<?php echo $boleta_deposito->estado->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-field="x_idboleta_deposito" name="x_idboleta_deposito" id="x_idboleta_deposito" value="<?php echo ew_HtmlEncode($boleta_deposito->idboleta_deposito->CurrentValue) ?>">
<?php
	if (in_array("pago_cliente", explode(",", $boleta_deposito->getCurrentDetailTable())) && $pago_cliente->DetailEdit) {
?>
...
<script type="text/javascript">
fboleta_depositoedit.Init();
</script>
<?php
$boleta_deposito_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$boleta_deposito_edit->Page_Terminate();
?>
