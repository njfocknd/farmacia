...
class cmoneda_list extends cmoneda {
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_moneda\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_moneda',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fmonedalist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
<?php
	...
	if ($moneda->CurrentAction == "" && $moneda_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$moneda_list->setWarningMessage($Language->Phrase("NoPermission"));
