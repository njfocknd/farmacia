...
$medicamento_add = NULL; // Initialize page object first

class cmedicamento_add extends cmedicamento {
	...
	// Page object name
	var $PageObjName = 'medicamento_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["idmedicamento"] != "") {
				...
				$this->setKey("idmedicamento", $this->idmedicamento->CurrentValue); // Set up key
			} else {
				$this->setKey("idmedicamento", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "medicamentoview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if (strval($this->idmarca->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@idmarca@", ew_AdjustSql($this->idmarca->CurrentValue), $sMasterFilter);
		} else {
		...
		if ($AddRow) {
			$this->idmedicamento->setDbValue($conn->Insert_ID());
			$rsnew['idmedicamento'] = $this->idmedicamento->DbValue;
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("registro_sanitario", $DetailTblVar) && $GLOBALS["registro_sanitario"]->DetailAdd) {
				$GLOBALS["registro_sanitario"]->idmedicamento->setSessionValue($this->idmedicamento->CurrentValue); // Set master key
				if (!isset($GLOBALS["registro_sanitario_grid"])) $GLOBALS["registro_sanitario_grid"] = new cregistro_sanitario_grid(); // Get detail page object
				$AddRow = $GLOBALS["registro_sanitario_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["registro_sanitario"]->idmedicamento->setSessionValue(""); // Clear master key if insert failed
			}
		}
...
// Create page object
if (!isset($medicamento_add)) $medicamento_add = new cmedicamento_add();

// Page init
$medicamento_add->Page_Init();

// Page main
$medicamento_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$medicamento_add->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var medicamento_add = new ew_Page("medicamento_add");
medicamento_add.PageID = "add"; // Page ID
var EW_PAGE_ID = medicamento_add.PageID; // For backward compatibility

// Form object
var fmedicamentoadd = new ew_Form("fmedicamentoadd");

// Validate form
fmedicamentoadd.Validate = function() {
...
fmedicamentoadd.Form_CustomValidate = 
...
fmedicamentoadd.ValidateRequired = true;
<?php } else { ?>
fmedicamentoadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmedicamentoadd.Lists["x_idpais"] = {"LinkField":"x_idpais","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fmedicamentoadd.Lists["x_idmarca"] = {"LinkField":"x_idmarca","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $medicamento_add->ShowPageHeader(); ?>
<?php
$medicamento_add->ShowMessage();
?>
<form name="fmedicamentoadd" id="fmedicamentoadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($medicamento_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $medicamento_add->Token ?>">
<?php } ?>
...
<?php
	if (in_array("registro_sanitario", explode(",", $medicamento->getCurrentDetailTable())) && $registro_sanitario->DetailAdd) {
?>
...
<script type="text/javascript">
fmedicamentoadd.Init();
</script>
<?php
$medicamento_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$medicamento_add->Page_Terminate();
?>
