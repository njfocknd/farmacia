...
$serie_documento_view = NULL; // Initialize page object first

class cserie_documento_view extends cserie_documento {
	...
	// Page object name
	var $PageObjName = 'serie_documento_view';
	...
	function __construct() {
		...
		if (@$_GET["idserie_documento"] <> "") {
			$this->RecKey["idserie_documento"] = $_GET["idserie_documento"];
			$KeyUrl .= "&amp;idserie_documento=" . urlencode($this->RecKey["idserie_documento"]);
		}
	...
	function Page_Init() {
		...
		if (!$Security->CanView()) {
			...
			$this->Page_Terminate(ew_GetUrl("serie_documentolist.php"));
		}
		...
		if (@$_GET["idserie_documento"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["idserie_documento"]);
		}
		...
		$this->idserie_documento->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["idserie_documento"] <> "") {
				...
				$this->RecKey["idserie_documento"] = $this->idserie_documento->QueryStringValue;
			} else {
				$sReturnUrl = "serie_documentolist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "serie_documentolist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "serie_documentolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$links = "";
		if ($GLOBALS["documento_debito_grid"] && $GLOBALS["documento_debito_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'documento_debito')) {
		...
		if ($GLOBALS["documento_debito_grid"] && $GLOBALS["documento_debito_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'documento_debito')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'documento_debito');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (documento_debito)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("documento_debito", explode(",", $this->getCurrentDetailTable()))) {
			global $documento_debito;
			if (!isset($documento_debito)) $documento_debito = new cdocumento_debito;
			$rsdetail = $documento_debito->LoadRs($documento_debito->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$documento_debito->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
...
// Create page object
if (!isset($serie_documento_view)) $serie_documento_view = new cserie_documento_view();

// Page init
$serie_documento_view->Page_Init();

// Page main
$serie_documento_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$serie_documento_view->Page_Render();
?>
...
// Page object
var serie_documento_view = new ew_Page("serie_documento_view");
serie_documento_view.PageID = "view"; // Page ID
var EW_PAGE_ID = serie_documento_view.PageID; // For backward compatibility

// Form object
var fserie_documentoview = new ew_Form("fserie_documentoview");

// Form_CustomValidate event
fserie_documentoview.Form_CustomValidate = 
...
fserie_documentoview.ValidateRequired = true;
<?php } else { ?>
fserie_documentoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fserie_documentoview.Lists["x_idtipo_documento"] = {"LinkField":"x_idtipo_documento","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fserie_documentoview.Lists["x_idsucursal"] = {"LinkField":"x_idsucursal","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php $serie_documento_view->ExportOptions->Render("body") ?>
<?php
	foreach ($serie_documento_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($serie_documento->Export == "") { ?>
...
<?php } ?>
<?php $serie_documento_view->ShowPageHeader(); ?>
<?php
$serie_documento_view->ShowMessage();
?>
<form name="fserie_documentoview" id="fserie_documentoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($serie_documento_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $serie_documento_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="serie_documento">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($serie_documento->idserie_documento->Visible) { // idserie_documento ?>
	<tr id="r_idserie_documento">
		<td><span id="elh_serie_documento_idserie_documento"><?php echo $serie_documento->idserie_documento->FldCaption() ?></span></td>
		<td<?php echo $serie_documento->idserie_documento->CellAttributes() ?>>
<span id="el_serie_documento_idserie_documento" class="form-group">
<span<?php echo $serie_documento->idserie_documento->ViewAttributes() ?>>
<?php echo $serie_documento->idserie_documento->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($serie_documento->idtipo_documento->Visible) { // idtipo_documento ?>
	<tr id="r_idtipo_documento">
		<td><span id="elh_serie_documento_idtipo_documento"><?php echo $serie_documento->idtipo_documento->FldCaption() ?></span></td>
...
<?php echo $serie_documento->idtipo_documento->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($serie_documento->idsucursal->Visible) { // idsucursal ?>
	<tr id="r_idsucursal">
		<td><span id="elh_serie_documento_idsucursal"><?php echo $serie_documento->idsucursal->FldCaption() ?></span></td>
...
<?php echo $serie_documento->idsucursal->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($serie_documento->serie->Visible) { // serie ?>
	<tr id="r_serie">
		<td><span id="elh_serie_documento_serie"><?php echo $serie_documento->serie->FldCaption() ?></span></td>
...
<?php echo $serie_documento->serie->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($serie_documento->correlativo->Visible) { // correlativo ?>
	<tr id="r_correlativo">
		<td><span id="elh_serie_documento_correlativo"><?php echo $serie_documento->correlativo->FldCaption() ?></span></td>
		<td<?php echo $serie_documento->correlativo->CellAttributes() ?>>
<span id="el_serie_documento_correlativo" class="form-group">
<span<?php echo $serie_documento->correlativo->ViewAttributes() ?>>
<?php echo $serie_documento->correlativo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($serie_documento->fecha->Visible) { // fecha ?>
	<tr id="r_fecha">
		<td><span id="elh_serie_documento_fecha"><?php echo $serie_documento->fecha->FldCaption() ?></span></td>
		<td<?php echo $serie_documento->fecha->CellAttributes() ?>>
<span id="el_serie_documento_fecha" class="form-group">
<span<?php echo $serie_documento->fecha->ViewAttributes() ?>>
<?php echo $serie_documento->fecha->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($serie_documento->estado->Visible) { // estado ?>
	<tr id="r_estado">
		<td><span id="elh_serie_documento_estado"><?php echo $serie_documento->estado->FldCaption() ?></span></td>
		<td<?php echo $serie_documento->estado->CellAttributes() ?>>
<span id="el_serie_documento_estado" class="form-group">
<span<?php echo $serie_documento->estado->ViewAttributes() ?>>
<?php echo $serie_documento->estado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fserie_documentoview.Init();
</script>
<?php
$serie_documento_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($serie_documento->Export == "") { ?>
...
<?php
$serie_documento_view->Page_Terminate();
?>
