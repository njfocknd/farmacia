...
$detalle_documento_debito_edit = NULL; // Initialize page object first

class cdetalle_documento_debito_edit extends cdetalle_documento_debito {
	...
	// Page object name
	var $PageObjName = 'detalle_documento_debito_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->iddetalle_documento_debito->CurrentValue == "")
			$this->Page_Terminate("detalle_documento_debitolist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		if (!$this->iddetalle_documento_debito->FldIsDetailKey)
...
// Create page object
if (!isset($detalle_documento_debito_edit)) $detalle_documento_debito_edit = new cdetalle_documento_debito_edit();

// Page init
$detalle_documento_debito_edit->Page_Init();

// Page main
$detalle_documento_debito_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detalle_documento_debito_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var detalle_documento_debito_edit = new ew_Page("detalle_documento_debito_edit");
detalle_documento_debito_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = detalle_documento_debito_edit.PageID; // For backward compatibility

// Form object
var fdetalle_documento_debitoedit = new ew_Form("fdetalle_documento_debitoedit");

// Validate form
fdetalle_documento_debitoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $detalle_documento_debito->estado->FldCaption(), $detalle_documento_debito->estado->ReqErrMsg)) ?>");
...
fdetalle_documento_debitoedit.Form_CustomValidate = 
...
fdetalle_documento_debitoedit.ValidateRequired = true;
<?php } else { ?>
fdetalle_documento_debitoedit.ValidateRequired = false; 
<?php } ?>
...
<?php $detalle_documento_debito_edit->ShowPageHeader(); ?>
<?php
$detalle_documento_debito_edit->ShowMessage();
?>
<form name="fdetalle_documento_debitoedit" id="fdetalle_documento_debitoedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($detalle_documento_debito_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $detalle_documento_debito_edit->Token ?>">
<?php } ?>
...
<?php if ($detalle_documento_debito->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_detalle_documento_debito_estado" for="x_estado" class="col-sm-2 control-label ewLabel"><?php echo $detalle_documento_debito->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $detalle_documento_debito->estado->CellAttributes() ?>>
<span id="el_detalle_documento_debito_estado">
<select data-field="x_estado" id="x_estado" name="x_estado"<?php echo $detalle_documento_debito->estado->EditAttributes() ?>>
<?php
if (is_array($detalle_documento_debito->estado->EditValue)) {
	$arwrk = $detalle_documento_debito->estado->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($detalle_documento_debito->estado->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<?php echo $detalle_documento_debito->estado->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-field="x_iddetalle_documento_debito" name="x_iddetalle_documento_debito" id="x_iddetalle_documento_debito" value="<?php echo ew_HtmlEncode($detalle_documento_debito->iddetalle_documento_debito->CurrentValue) ?>">
...
<script type="text/javascript">
fdetalle_documento_debitoedit.Init();
</script>
<?php
$detalle_documento_debito_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$detalle_documento_debito_edit->Page_Terminate();
?>
