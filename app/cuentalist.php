...
class ccuenta_list extends ccuenta {
	...
	function SetupListOptions() {
		...
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'cuenta_transaccion') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList(CurrentProjectID() . 'cuenta_transaccion')) {
			...
			$links = "";
			if ($GLOBALS["cuenta_transaccion_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'cuenta_transaccion')) {
			...
			if ($GLOBALS["cuenta_transaccion_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'cuenta_transaccion')) {
	...
	function SetupOtherOptions() {
		...
		$item->Visible = ($GLOBALS["cuenta_transaccion"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'cuenta_transaccion') && $Security->CanAdd());
		if ($item->Visible) {
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_cuenta\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_cuenta',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fcuentalist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "banco") {
			global $banco;
			if (!isset($banco)) $banco = new cbanco;
			$rsmaster = $banco->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$banco->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
				}
...
<?php
	...
	if ($cuenta->CurrentAction == "" && $cuenta_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$cuenta_list->setWarningMessage($Language->Phrase("NoPermission"));
