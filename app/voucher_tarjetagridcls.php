...
$voucher_tarjeta_grid = NULL; // Initialize page object first

class cvoucher_tarjeta_grid extends cvoucher_tarjeta {
	...
	// Page object name
	var $PageObjName = 'voucher_tarjeta_grid';

	// Grid form hidden field names
	var $FormName = 'fvoucher_tarjetagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["voucher_tarjeta"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idvoucher_tarjeta->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_nombre") && $objForm->HasValue("o_nombre") && $this->nombre->CurrentValue <> $this->nombre->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_ultimos_cuatro_digitos") && $objForm->HasValue("o_ultimos_cuatro_digitos") && $this->ultimos_cuatro_digitos->CurrentValue <> $this->ultimos_cuatro_digitos->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_referencia") && $objForm->HasValue("o_referencia") && $this->referencia->CurrentValue <> $this->referencia->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idvoucher_tarjeta->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idvoucher_tarjeta');
	}
	...
	function LoadDefaultValues() {
		...
		$this->ultimos_cuatro_digitos->OldValue = $this->ultimos_cuatro_digitos->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->ultimos_cuatro_digitos->setOldValue($objForm->GetValue("o_ultimos_cuatro_digitos"));
		if (!$this->referencia->FldIsDetailKey) {
			$this->referencia->setFormValue($objForm->GetValue("x_referencia"));
		}
		$this->referencia->setOldValue($objForm->GetValue("o_referencia"));
		...
		if (!$this->idvoucher_tarjeta->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idvoucher_tarjeta->CurrentValue = strval($arKeys[0]); // idvoucher_tarjeta
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['idvoucher_tarjeta'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
