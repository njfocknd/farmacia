...
class ccategoria_list extends ccategoria {
	...
	function __construct() {
		...
		$this->AddUrl = "categoriaadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idcategoria->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idcategoria->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function SetupListOptions() {
		...
		// "detail_producto"
		$item = &$this->ListOptions->Add("detail_producto");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'producto') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
	...
	function RenderListOptions() {
		...
		// "detail_producto"
		$oListOpt = &$this->ListOptions->Items["detail_producto"];
		if ($Security->AllowList(CurrentProjectID() . 'producto')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("producto", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("productolist.php?" . EW_TABLE_SHOW_MASTER . "=categoria&fk_idcategoria=" . strval($this->idcategoria->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["producto_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'producto')) {
			...
			if ($GLOBALS["producto_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'producto')) {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->idcategoria->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'>";
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_producto");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=producto") . "\">" . $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["producto"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["producto"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'producto') && $Security->CanAdd());
		if ($item->Visible) {
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fcategorialist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcategorialistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("idcategoria")) <> "")
			$this->idcategoria->CurrentValue = $this->getKey("idcategoria"); // idcategoria
		else
			$bValidKey = FALSE;
...
// Form object
var fcategorialist = new ew_Form("fcategorialist");
...
fcategorialist.ValidateRequired = true;
<?php } else { ?>
fcategorialist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var fcategorialistsrch = new ew_Form("fcategorialistsrch");
</script>
<script type="text/javascript">
...
<?php
	...
	if ($categoria->CurrentAction == "" && $categoria_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$categoria_list->setWarningMessage($Language->Phrase("NoPermission"));
...
<form name="fcategorialistsrch" id="fcategorialistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
...
<?php } ?>
<input type="hidden" name="t" value="categoria">
<div id="gmp_categoria" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
...
<?php if ($categoria->nombre->Visible) { // nombre ?>
	<?php if ($categoria->SortUrl($categoria->nombre) == "") { ?>
		<th data-name="nombre"><div id="elh_categoria_nombre" class="categoria_nombre"><div class="ewTableHeaderCaption"><?php echo $categoria->nombre->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nombre"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $categoria->SortUrl($categoria->nombre) ?>',1);"><div id="elh_categoria_nombre" class="categoria_nombre">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $categoria->nombre->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($categoria->nombre->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($categoria->nombre->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<a id="<?php echo $categoria_list->PageObjName . "_row_" . $categoria_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php
