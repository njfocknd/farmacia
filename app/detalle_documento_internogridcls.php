...
$detalle_documento_interno_grid = NULL; // Initialize page object first

class cdetalle_documento_interno_grid extends cdetalle_documento_interno {
	...
	// Page object name
	var $PageObjName = 'detalle_documento_interno_grid';

	// Grid form hidden field names
	var $FormName = 'fdetalle_documento_internogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detalle_documento_interno"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->iddetalle_documento_interno->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_iddocumento_interno") && $objForm->HasValue("o_iddocumento_interno") && $this->iddocumento_interno->CurrentValue <> $this->iddocumento_interno->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idproducto") && $objForm->HasValue("o_idproducto") && $this->idproducto->CurrentValue <> $this->idproducto->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idbodega_ingreso") && $objForm->HasValue("o_idbodega_ingreso") && $this->idbodega_ingreso->CurrentValue <> $this->idbodega_ingreso->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->iddetalle_documento_interno->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('iddetalle_documento_interno');
	}
	...
	function LoadFormValues() {
		...
		$this->iddocumento_interno->setOldValue($objForm->GetValue("o_iddocumento_interno"));
		...
		$this->idbodega_ingreso->setOldValue($objForm->GetValue("o_idbodega_ingreso"));
		if (!$this->idbodega_egreso->FldIsDetailKey) {
			$this->idbodega_egreso->setFormValue($objForm->GetValue("x_idbodega_egreso"));
		}
		$this->idbodega_egreso->setOldValue($objForm->GetValue("o_idbodega_egreso"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddetalle_documento_interno->CurrentValue = strval($arKeys[0]); // iddetalle_documento_interno
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['iddetalle_documento_interno'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// iddocumento_interno
			$this->iddocumento_interno->SetDbValueDef($rsnew, $this->iddocumento_interno->CurrentValue, 0, $this->iddocumento_interno->ReadOnly);

			// idproducto
			$this->idproducto->SetDbValueDef($rsnew, $this->idproducto->CurrentValue, 0, $this->idproducto->ReadOnly);

			// idbodega_ingreso
			$this->idbodega_ingreso->SetDbValueDef($rsnew, $this->idbodega_ingreso->CurrentValue, 0, $this->idbodega_ingreso->ReadOnly);

			// idbodega_egreso
			$this->idbodega_egreso->SetDbValueDef($rsnew, $this->idbodega_egreso->CurrentValue, 0, $this->idbodega_egreso->ReadOnly);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "documento_interno") {
			...
			if ($GLOBALS["documento_interno"]->EventCancelled) $this->EventCancelled = TRUE;
		}
