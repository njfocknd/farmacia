...
class cbanco_list extends cbanco {
	...
	function SetupListOptions() {
		...
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'cuenta') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList(CurrentProjectID() . 'cuenta')) {
			...
			$links = "";
			if ($GLOBALS["cuenta_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'cuenta')) {
			...
			if ($GLOBALS["cuenta_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'cuenta')) {
	...
	function SetupOtherOptions() {
		...
		$item->Visible = ($GLOBALS["cuenta"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'cuenta') && $Security->CanAdd());
		if ($item->Visible) {
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_banco\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_banco',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fbancolist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
<?php
	...
	if ($banco->CurrentAction == "" && $banco_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$banco_list->setWarningMessage($Language->Phrase("NoPermission"));
