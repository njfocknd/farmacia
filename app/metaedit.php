...
$meta_edit = NULL; // Initialize page object first

class cmeta_edit extends cmeta {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idmeta->CurrentValue == "")
			$this->Page_Terminate("metalist.php"); // Invalid key, return to list
...
// Create page object
if (!isset($meta_edit)) $meta_edit = new cmeta_edit();
...
$meta_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var meta_edit = new ew_Page("meta_edit");
meta_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = meta_edit.PageID; // For backward compatibility

// Form object
var fmetaedit = new ew_Form("fmetaedit");

// Validate form
fmetaedit.Validate = function() {
...
fmetaedit.ValidateRequired = true;
<?php } else { ?>
fmetaedit.ValidateRequired = false; 
<?php } ?>
...
<?php $meta_edit->ShowPageHeader(); ?>
<?php
$meta_edit->ShowMessage();
?>
<form name="fmetaedit" id="fmetaedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($meta_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $meta_edit->Token ?>">
<?php } ?>
...
<?php if ($meta->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_meta_estado" for="x_estado" class="col-sm-2 control-label ewLabel"><?php echo $meta->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $meta->estado->CellAttributes() ?>>
<span id="el_meta_estado">
<select data-field="x_estado" id="x_estado" name="x_estado"<?php echo $meta->estado->EditAttributes() ?>>
<?php
if (is_array($meta->estado->EditValue)) {
	$arwrk = $meta->estado->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($meta->estado->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<?php echo $meta->estado->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-field="x_idmeta" name="x_idmeta" id="x_idmeta" value="<?php echo ew_HtmlEncode($meta->idmeta->CurrentValue) ?>">
