...
class cpais_view extends cpais {
	...
	function SetupOtherOptions() {
		...
		$links = "";
		if ($GLOBALS["departamento_grid"] && $GLOBALS["departamento_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'departamento')) {
		...
		if ($GLOBALS["departamento_grid"] && $GLOBALS["departamento_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'departamento')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'moneda');
		if ($item->Visible) {
		...
		$links = "";
		if ($GLOBALS["moneda_grid"] && $GLOBALS["moneda_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'moneda')) {
		...
		if ($GLOBALS["moneda_grid"] && $GLOBALS["moneda_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'moneda')) {
	...
	function ExportData() {
		...
		// Export detail records (moneda)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("moneda", explode(",", $this->getCurrentDetailTable()))) {
			global $moneda;
			if (!isset($moneda)) $moneda = new cmoneda;
			$rsdetail = $moneda->LoadRs($moneda->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("h"); // Change to horizontal
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					$Doc->ExportEmptyRow();
					$detailcnt = $rsdetail->RecordCount();
					$moneda->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
				$Doc->SetStyle($ExportStyle); // Restore
				$rsdetail->Close();
			}
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("moneda", $DetailTblVar)) {
				...
				if ($GLOBALS["moneda_grid"]->DetailView) {
					$GLOBALS["moneda_grid"]->CurrentMode = "view";
...
<?php } ?>
<?php
	if (in_array("moneda", explode(",", $pais->getCurrentDetailTable())) && $moneda->DetailView) {
?>
<?php if ($pais->getCurrentDetailTable() <> "") { ?>
