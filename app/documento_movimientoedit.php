...
$documento_movimiento_edit = NULL; // Initialize page object first

class cdocumento_movimiento_edit extends cdocumento_movimiento {
	...
	// Page object name
	var $PageObjName = 'documento_movimiento_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->iddocumento_movimiento->CurrentValue == "")
			$this->Page_Terminate("documento_movimientolist.php"); // Invalid key, return to list
	...
	function WriteAuditTrailOnEdit(&$rsold, &$rsnew) {
		...
		$key .= $rsold['iddocumento_movimiento'];
...
// Create page object
if (!isset($documento_movimiento_edit)) $documento_movimiento_edit = new cdocumento_movimiento_edit();

// Page init
$documento_movimiento_edit->Page_Init();

// Page main
$documento_movimiento_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$documento_movimiento_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var documento_movimiento_edit = new ew_Page("documento_movimiento_edit");
documento_movimiento_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = documento_movimiento_edit.PageID; // For backward compatibility

// Form object
var fdocumento_movimientoedit = new ew_Form("fdocumento_movimientoedit");

// Validate form
fdocumento_movimientoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $documento_movimiento->estado_documento->FldCaption(), $documento_movimiento->estado_documento->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_estado");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $documento_movimiento->estado->FldCaption(), $documento_movimiento->estado->ReqErrMsg)) ?>");
...
fdocumento_movimientoedit.Form_CustomValidate = 
...
fdocumento_movimientoedit.ValidateRequired = true;
<?php } else { ?>
fdocumento_movimientoedit.ValidateRequired = false; 
<?php } ?>
...
<?php $documento_movimiento_edit->ShowPageHeader(); ?>
<?php
$documento_movimiento_edit->ShowMessage();
?>
<form name="fdocumento_movimientoedit" id="fdocumento_movimientoedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($documento_movimiento_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $documento_movimiento_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fdocumento_movimientoedit", "x_fecha", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<?php if ($documento_movimiento->estado_documento->Visible) { // estado_documento ?>
	<div id="r_estado_documento" class="form-group">
		<label id="elh_documento_movimiento_estado_documento" for="x_estado_documento" class="col-sm-2 control-label ewLabel"><?php echo $documento_movimiento->estado_documento->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $documento_movimiento->estado_documento->CellAttributes() ?>>
<span id="el_documento_movimiento_estado_documento">
<select data-field="x_estado_documento" id="x_estado_documento" name="x_estado_documento"<?php echo $documento_movimiento->estado_documento->EditAttributes() ?>>
<?php
if (is_array($documento_movimiento->estado_documento->EditValue)) {
	$arwrk = $documento_movimiento->estado_documento->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($documento_movimiento->estado_documento->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<?php echo $documento_movimiento->estado_documento->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($documento_movimiento->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_documento_movimiento_estado" class="col-sm-2 control-label ewLabel"><?php echo $documento_movimiento->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $documento_movimiento->estado->CellAttributes() ?>>
<span id="el_documento_movimiento_estado">
<div id="tp_x_estado" class="<?php echo EW_ITEM_TEMPLATE_CLASSNAME ?>"><input type="radio" name="x_estado" id="x_estado" value="{value}"<?php echo $documento_movimiento->estado->EditAttributes() ?>></div>
<div id="dsl_x_estado" data-repeatcolumn="5" class="ewItemList">
<?php
$arwrk = $documento_movimiento->estado->EditValue;
if (is_array($arwrk)) {
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($documento_movimiento->estado->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;

		// Note: No spacing within the LABEL tag
?>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 1) ?>
<label class="radio-inline"><input type="radio" data-field="x_estado" name="x_estado" id="x_estado_<?php echo $rowcntwrk ?>" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $documento_movimiento->estado->EditAttributes() ?>><?php echo $arwrk[$rowcntwrk][1] ?></label>
...
<?php echo $documento_movimiento->estado->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<input type="hidden" data-field="x_iddocumento_movimiento" name="x_iddocumento_movimiento" id="x_iddocumento_movimiento" value="<?php echo ew_HtmlEncode($documento_movimiento->iddocumento_movimiento->CurrentValue) ?>">
...
<script type="text/javascript">
fdocumento_movimientoedit.Init();
</script>
<?php
$documento_movimiento_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$documento_movimiento_edit->Page_Terminate();
?>
