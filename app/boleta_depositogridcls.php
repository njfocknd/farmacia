...
$boleta_deposito_grid = NULL; // Initialize page object first

class cboleta_deposito_grid extends cboleta_deposito {
	...
	// Page object name
	var $PageObjName = 'boleta_deposito_grid';

	// Grid form hidden field names
	var $FormName = 'fboleta_depositogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["boleta_deposito"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idboleta_deposito->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idboleta_deposito->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idboleta_deposito');
	}
	...
	function LoadFormValues() {
		...
		if (!$this->idboleta_deposito->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idboleta_deposito->CurrentValue = strval($arKeys[0]); // idboleta_deposito
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['idboleta_deposito'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
