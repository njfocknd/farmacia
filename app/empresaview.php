...
class cempresa_view extends cempresa {
	...
	function Page_Init() {
		...
		if (!$Security->CanView()) {
			...
			$this->Page_Terminate(ew_GetUrl("empresalist.php"));
		}
		...
		if (@$_GET["idempresa"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["idempresa"]);
		}
	...
	function SetupOtherOptions() {
		...
		$links = "";
		if ($GLOBALS["sucursal_grid"] && $GLOBALS["sucursal_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'sucursal')) {
		...
		if ($GLOBALS["sucursal_grid"] && $GLOBALS["sucursal_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'sucursal')) {
	...
	function ExportData() {
		...
		// Export detail records (sucursal)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("sucursal", explode(",", $this->getCurrentDetailTable()))) {
			global $sucursal;
			if (!isset($sucursal)) $sucursal = new csucursal;
			$rsdetail = $sucursal->LoadRs($sucursal->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$sucursal->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
