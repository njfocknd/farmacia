...
$medicamento_edit = NULL; // Initialize page object first

class cmedicamento_edit extends cmedicamento {
	...
	// Page object name
	var $PageObjName = 'medicamento_edit';
	...
	function Page_Main() {
		...
		if (@$_GET["idmedicamento"] <> "") {
			$this->idmedicamento->setQueryStringValue($_GET["idmedicamento"]);
		}
		...
		// Check if valid key
		if ($this->idmedicamento->CurrentValue == "")
			$this->Page_Terminate("medicamentolist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("medicamentolist.php"); // No matching record, return to list
				}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Check referential integrity for master table 'marca'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_marca();
			$KeyValue = isset($rsnew['idmarca']) ? $rsnew['idmarca'] : $rsold['idmarca'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@idmarca@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
				$bValidMasterRecord = FALSE;
			}
			if ($bValidMasterRecord) {
				$rsmaster = $GLOBALS["marca"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "marca", $Language->Phrase("RelatedRecordRequired"));
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("registro_sanitario", $DetailTblVar)) {
				...
				if ($GLOBALS["registro_sanitario_grid"]->DetailEdit) {
					...
					$GLOBALS["registro_sanitario_grid"]->idmedicamento->FldIsDetailKey = TRUE;
					$GLOBALS["registro_sanitario_grid"]->idmedicamento->CurrentValue = $this->idmedicamento->CurrentValue;
					$GLOBALS["registro_sanitario_grid"]->idmedicamento->setSessionValue($GLOBALS["registro_sanitario_grid"]->idmedicamento->CurrentValue);
				}
			}
		}
	}
...
// Create page object
if (!isset($medicamento_edit)) $medicamento_edit = new cmedicamento_edit();

// Page init
$medicamento_edit->Page_Init();

// Page main
$medicamento_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$medicamento_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var medicamento_edit = new ew_Page("medicamento_edit");
medicamento_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = medicamento_edit.PageID; // For backward compatibility

// Form object
var fmedicamentoedit = new ew_Form("fmedicamentoedit");

// Validate form
fmedicamentoedit.Validate = function() {
...
fmedicamentoedit.Form_CustomValidate = 
...
fmedicamentoedit.ValidateRequired = true;
<?php } else { ?>
fmedicamentoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmedicamentoedit.Lists["x_idpais"] = {"LinkField":"x_idpais","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fmedicamentoedit.Lists["x_idmarca"] = {"LinkField":"x_idmarca","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $medicamento_edit->ShowPageHeader(); ?>
<?php
$medicamento_edit->ShowMessage();
?>
<form name="fmedicamentoedit" id="fmedicamentoedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($medicamento_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $medicamento_edit->Token ?>">
<?php } ?>
...
<?php if ($medicamento->nombre->Visible) { // nombre ?>
	<div id="r_nombre" class="form-group">
		<label id="elh_medicamento_nombre" for="x_nombre" class="col-sm-2 control-label ewLabel"><?php echo $medicamento->nombre->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $medicamento->nombre->CellAttributes() ?>>
<span id="el_medicamento_nombre">
<input type="text" data-field="x_nombre" name="x_nombre" id="x_nombre" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($medicamento->nombre->PlaceHolder) ?>" value="<?php echo $medicamento->nombre->EditValue ?>"<?php echo $medicamento->nombre->EditAttributes() ?>>
</span>
<?php echo $medicamento->nombre->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($medicamento->idpais->Visible) { // idpais ?>
	<div id="r_idpais" class="form-group">
		<label id="elh_medicamento_idpais" for="x_idpais" class="col-sm-2 control-label ewLabel"><?php echo $medicamento->idpais->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $medicamento->idpais->CellAttributes() ?>>
<span id="el_medicamento_idpais">
<select data-field="x_idpais" id="x_idpais" name="x_idpais"<?php echo $medicamento->idpais->EditAttributes() ?>>
<?php
...
<?php echo $medicamento->idpais->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($medicamento->idmarca->Visible) { // idmarca ?>
	<div id="r_idmarca" class="form-group">
		<label id="elh_medicamento_idmarca" for="x_idmarca" class="col-sm-2 control-label ewLabel"><?php echo $medicamento->idmarca->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $medicamento->idmarca->CellAttributes() ?>>
<?php if ($medicamento->idmarca->getSessionValue() <> "") { ?>
<span id="el_medicamento_idmarca">
...
<input type="hidden" id="x_idmarca" name="x_idmarca" value="<?php echo ew_HtmlEncode($medicamento->idmarca->CurrentValue) ?>">
<?php } else { ?>
<span id="el_medicamento_idmarca">
<select data-field="x_idmarca" id="x_idmarca" name="x_idmarca"<?php echo $medicamento->idmarca->EditAttributes() ?>>
<?php
...
<?php } ?>
<?php echo $medicamento->idmarca->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($medicamento->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_medicamento_estado" for="x_estado" class="col-sm-2 control-label ewLabel"><?php echo $medicamento->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $medicamento->estado->CellAttributes() ?>>
<span id="el_medicamento_estado">
<select data-field="x_estado" id="x_estado" name="x_estado"<?php echo $medicamento->estado->EditAttributes() ?>>
<?php
...
<?php echo $medicamento->estado->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-field="x_idmedicamento" name="x_idmedicamento" id="x_idmedicamento" value="<?php echo ew_HtmlEncode($medicamento->idmedicamento->CurrentValue) ?>">
<?php
	if (in_array("registro_sanitario", explode(",", $medicamento->getCurrentDetailTable())) && $registro_sanitario->DetailEdit) {
?>
<?php if ($medicamento->getCurrentDetailTable() <> "") { ?>
...
<script type="text/javascript">
fmedicamentoedit.Init();
</script>
<?php
$medicamento_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$medicamento_edit->Page_Terminate();
?>
