...
$medicamento_grid = NULL; // Initialize page object first

class cmedicamento_grid extends cmedicamento {
	...
	// Page object name
	var $PageObjName = 'medicamento_grid';

	// Grid form hidden field names
	var $FormName = 'fmedicamentogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["medicamento"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idmedicamento->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idmedicamento->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->idmedicamento->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idmedicamento->CurrentValue = strval($arKeys[0]); // idmedicamento
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
