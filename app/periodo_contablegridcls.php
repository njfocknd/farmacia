...
$periodo_contable_grid = NULL; // Initialize page object first

class cperiodo_contable_grid extends cperiodo_contable {
	...
	// Page object name
	var $PageObjName = 'periodo_contable_grid';

	// Grid form hidden field names
	var $FormName = 'fperiodo_contablegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["periodo_contable"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idperiodo_contable->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idperiodo_contable->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idperiodo_contable');
	}
	...
	function LoadFormValues() {
		...
		if (!$this->idperiodo_contable->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idperiodo_contable->CurrentValue = strval($arKeys[0]); // idperiodo_contable
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['idperiodo_contable'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// anio
			$this->anio->SetDbValueDef($rsnew, $this->anio->CurrentValue, "", $this->anio->ReadOnly);
