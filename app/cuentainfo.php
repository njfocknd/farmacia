...
class ccuenta extends cTable {
	...
	function __construct() {
		...
		// fecha_insercio
		$this->fecha_insercio = new cField('cuenta', 'cuenta', 'x_fecha_insercio', 'fecha_insercio', '`fecha_insercio`', 'DATE_FORMAT(`fecha_insercio`, \'%d/%m/%Y\')', 135, 7, FALSE, '`fecha_insercio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha_insercio->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha_insercio'] = &$this->fecha_insercio;
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->fecha_insercio->Exportable) $Doc->ExportCaption($this->fecha_insercio);
				} else {
					if ($this->idcuenta->Exportable) $Doc->ExportCaption($this->idcuenta);
					if ($this->idbanco->Exportable) $Doc->ExportCaption($this->idbanco);
					if ($this->idsucursal->Exportable) $Doc->ExportCaption($this->idsucursal);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->fecha_insercio->Exportable) $Doc->ExportField($this->fecha_insercio);
					} else {
						if ($this->idcuenta->Exportable) $Doc->ExportField($this->idcuenta);
						if ($this->idbanco->Exportable) $Doc->ExportField($this->idbanco);
						if ($this->idsucursal->Exportable) $Doc->ExportField($this->idsucursal);
