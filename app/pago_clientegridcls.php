...
$pago_cliente_grid = NULL; // Initialize page object first

class cpago_cliente_grid extends cpago_cliente {
	...
	// Page object name
	var $PageObjName = 'pago_cliente_grid';

	// Grid form hidden field names
	var $FormName = 'fpago_clientegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["pago_cliente"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idpago_cliente->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_idtipo_pago") && $objForm->HasValue("o_idtipo_pago") && $this->idtipo_pago->CurrentValue <> $this->idtipo_pago->OldValue)
			return FALSE;
		...
		if ($objForm->HasValue("x_idsucursal") && $objForm->HasValue("o_idsucursal") && $this->idsucursal->CurrentValue <> $this->idsucursal->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idboleta_deposito") && $objForm->HasValue("o_idboleta_deposito") && $this->idboleta_deposito->CurrentValue <> $this->idboleta_deposito->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idvoucher_tarjeta") && $objForm->HasValue("o_idvoucher_tarjeta") && $this->idvoucher_tarjeta->CurrentValue <> $this->idvoucher_tarjeta->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idcheque_cliente") && $objForm->HasValue("o_idcheque_cliente") && $this->idcheque_cliente->CurrentValue <> $this->idcheque_cliente->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idpago_cliente->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->idtipo_pago->OldValue = $this->idtipo_pago->CurrentValue;
		...
		$this->idboleta_deposito->CurrentValue = NULL;
		$this->idboleta_deposito->OldValue = $this->idboleta_deposito->CurrentValue;
		$this->idvoucher_tarjeta->CurrentValue = NULL;
		$this->idvoucher_tarjeta->OldValue = $this->idvoucher_tarjeta->CurrentValue;
		$this->idcheque_cliente->CurrentValue = NULL;
		$this->idcheque_cliente->OldValue = $this->idcheque_cliente->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->idtipo_pago->setOldValue($objForm->GetValue("o_idtipo_pago"));
		...
		$this->idboleta_deposito->setOldValue($objForm->GetValue("o_idboleta_deposito"));
		if (!$this->idvoucher_tarjeta->FldIsDetailKey) {
			$this->idvoucher_tarjeta->setFormValue($objForm->GetValue("x_idvoucher_tarjeta"));
		}
		$this->idvoucher_tarjeta->setOldValue($objForm->GetValue("o_idvoucher_tarjeta"));
		if (!$this->idcheque_cliente->FldIsDetailKey) {
			$this->idcheque_cliente->setFormValue($objForm->GetValue("x_idcheque_cliente"));
		}
		$this->idcheque_cliente->setOldValue($objForm->GetValue("o_idcheque_cliente"));
		if (!$this->idpago_cliente->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idpago_cliente->CurrentValue = strval($arKeys[0]); // idpago_cliente
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->monto->EditValue = ew_FormatNumber($this->monto->EditValue, -2, -2, 0, -1);
			...
			if (trim(strval($this->idcheque_cliente->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`idcheque_cliente`" . ew_SearchString("=", $this->idcheque_cliente->CurrentValue, EW_DATATYPE_NUMBER);
			}
			$sSqlWrk = "SELECT `idcheque_cliente`, `numero` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `cheque_cliente`";
			$sWhereWrk = "";
			$lookuptblfilter = "`estado` = 'Activo'";
			if (strval($lookuptblfilter) <> "") {
				ew_AddFilter($sWhereWrk, $lookuptblfilter);
			...
			if ($rswrk) $rswrk->Close();
			array_unshift($arwrk, array("", $Language->Phrase("PleaseSelect"), "", "", "", "", "", "", ""));
			$this->idcheque_cliente->EditValue = $arwrk;
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// idtipo_pago
			$this->idtipo_pago->SetDbValueDef($rsnew, $this->idtipo_pago->CurrentValue, 0, $this->idtipo_pago->ReadOnly);
			...
			// idcheque_cliente
			$this->idcheque_cliente->SetDbValueDef($rsnew, $this->idcheque_cliente->CurrentValue, NULL, $this->idcheque_cliente->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		// idboleta_deposito
		$this->idboleta_deposito->SetDbValueDef($rsnew, $this->idboleta_deposito->CurrentValue, NULL, FALSE);

		// idvoucher_tarjeta
		$this->idvoucher_tarjeta->SetDbValueDef($rsnew, $this->idvoucher_tarjeta->CurrentValue, NULL, FALSE);

		// idcheque_cliente
		$this->idcheque_cliente->SetDbValueDef($rsnew, $this->idcheque_cliente->CurrentValue, NULL, FALSE);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "cliente") {
			...
			if ($GLOBALS["cliente"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "tipo_pago") {
			$this->idtipo_pago->Visible = FALSE;
			if ($GLOBALS["tipo_pago"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "boleta_deposito") {
			$this->idboleta_deposito->Visible = FALSE;
			if ($GLOBALS["boleta_deposito"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "voucher_tarjeta") {
			$this->idvoucher_tarjeta->Visible = FALSE;
			if ($GLOBALS["voucher_tarjeta"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "cheque_cliente") {
			$this->idcheque_cliente->Visible = FALSE;
			if ($GLOBALS["cheque_cliente"]->EventCancelled) $this->EventCancelled = TRUE;
		}
