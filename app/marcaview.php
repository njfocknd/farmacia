...
class cmarca_view extends cmarca {
	...
	function SetupOtherOptions() {
		...
		// "detail_medicamento"
		$item = &$option->Add("detail_medicamento");
		...
		$links = "";
		if ($GLOBALS["medicamento_grid"] && $GLOBALS["medicamento_grid"]->DetailView) {
		...
		if ($GLOBALS["medicamento_grid"] && $GLOBALS["medicamento_grid"]->DetailEdit) {
	...
	function ExportData() {
		...
		// Export detail records (medicamento)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("medicamento", explode(",", $this->getCurrentDetailTable()))) {
			global $medicamento;
			if (!isset($medicamento)) $medicamento = new cmedicamento;
			$rsdetail = $medicamento->LoadRs($medicamento->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$medicamento->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("medicamento", $DetailTblVar)) {
				...
				if ($GLOBALS["medicamento_grid"]->DetailView) {
					$GLOBALS["medicamento_grid"]->CurrentMode = "view";
...
fmarcaview.Lists["x_idlaboratorio"] = {"LinkField":"x_idlaboratorio","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if ($marca->idlaboratorio->Visible) { // idlaboratorio ?>
	<tr id="r_idlaboratorio">
		<td><span id="elh_marca_idlaboratorio"><?php echo $marca->idlaboratorio->FldCaption() ?></span></td>
...
<?php echo $marca->idlaboratorio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("medicamento", explode(",", $marca->getCurrentDetailTable())) && $medicamento->DetailView) {
?>
