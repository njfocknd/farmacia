...
class caudittrail_list extends caudittrail {
	...
	function SetupSearchOptions() {
		...
		if (ew_IsMobile())
			...
			$item->Body = "<button type=\"button\" class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" onclick=\"ew_SearchDialogShow({lnk:this,url:'audittrailsrch.php'});\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;
...
faudittraillist.Lists["x_user"] = {"LinkField":"x_idusuario","Ajax":true,"AutoFill":false,"DisplayFields":["x_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if ($audittrail->user->Visible) { // user ?>
	...
	<?php } else { ?>
		<th data-name="user"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $audittrail->SortUrl($audittrail->user) ?>',1);"><div id="elh_audittrail_user" class="audittrail_user">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $audittrail->user->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($audittrail->user->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($audittrail->user->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
