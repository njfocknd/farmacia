...
$marca_grid = NULL; // Initialize page object first

class cmarca_grid extends cmarca {
	...
	// Page object name
	var $PageObjName = 'marca_grid';

	// Grid form hidden field names
	var $FormName = 'fmarcagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["marca"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idmarca->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idmarca->CurrentValue . "\">";
		}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idmarca->CurrentValue = strval($arKeys[0]); // idmarca
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
