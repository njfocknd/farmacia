...
//
// Table class for bodega
//
class cbodega extends cTable {
	var $idbodega;
	var $descripcion;
	var $idsucursal;
	var $idtipo_bodega;
	...
	function __construct() {
		...
		// idbodega
		$this->idbodega = new cField('bodega', 'bodega', 'x_idbodega', 'idbodega', '`idbodega`', '`idbodega`', 3, -1, FALSE, '`idbodega`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idbodega->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idbodega'] = &$this->idbodega;

		// descripcion
		$this->descripcion = new cField('bodega', 'bodega', 'x_descripcion', 'descripcion', '`descripcion`', '`descripcion`', 200, -1, FALSE, '`descripcion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['descripcion'] = &$this->descripcion;

		// idsucursal
		$this->idsucursal = new cField('bodega', 'bodega', 'x_idsucursal', 'idsucursal', '`idsucursal`', '`idsucursal`', 3, -1, FALSE, '`idsucursal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idsucursal->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idsucursal'] = &$this->idsucursal;

		// idtipo_bodega
		$this->idtipo_bodega = new cField('bodega', 'bodega', 'x_idtipo_bodega', 'idtipo_bodega', '`idtipo_bodega`', '`idtipo_bodega`', 3, -1, FALSE, '`idtipo_bodega`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idtipo_bodega->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idtipo_bodega'] = &$this->idtipo_bodega;

		// estado
		$this->estado = new cField('bodega', 'bodega', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['estado'] = &$this->estado;

		// fecha_insercion
		$this->fecha_insercion = new cField('bodega', 'bodega', 'x_fecha_insercion', 'fecha_insercion', '`fecha_insercion`', 'DATE_FORMAT(`fecha_insercion`, \'%d/%m/%Y\')', 135, 7, FALSE, '`fecha_insercion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "tipo_bodega") {
			if ($this->idtipo_bodega->getSessionValue() <> "")
				$sMasterFilter .= "`idtipo_bodega`=" . ew_QuotedValue($this->idtipo_bodega->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "tipo_bodega") {
			if ($this->idtipo_bodega->getSessionValue() <> "")
				$sDetailFilter .= "`idtipo_bodega`=" . ew_QuotedValue($this->idtipo_bodega->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "sucursal") {
	...
	// Master filter
	function SqlMasterFilter_tipo_bodega() {
		return "`idtipo_bodega`=@idtipo_bodega@";
	}

	// Detail filter
	function SqlDetailFilter_tipo_bodega() {
		return "`idtipo_bodega`=@idtipo_bodega@";
	}
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "producto_bodega") {
			...
			$sDetailUrl .= "&fk_idbodega=" . urlencode($this->idbodega->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "bodegalist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`bodega`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('idbodega', $rs))
				ew_AddFilter($where, ew_QuotedName('idbodega') . '=' . ew_QuotedValue($rs['idbodega'], $this->idbodega->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->idbodega->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idbodega@", ew_AdjustSql($this->idbodega->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("bodegaview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("bodegaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "bodegaadd.php?" . $this->UrlParm($parm);
		else
			return "bodegaadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("bodegaedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("bodegaedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("bodegaadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("bodegaadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("bodegadelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idbodega->CurrentValue)) {
			$sUrl .= "idbodega=" . urlencode($this->idbodega->CurrentValue);
		} else {
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->idbodega->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->idbodega->EditValue = $this->idbodega->CurrentValue;
		$this->idbodega->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->idtipo_bodega->Exportable) $Doc->ExportCaption($this->idtipo_bodega);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->idtipo_bodega->Exportable) $Doc->ExportField($this->idtipo_bodega);
