...
class cproveedor_list extends cproveedor {
	...
	function SetupListOptions() {
		...
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'pago_proveedor') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList(CurrentProjectID() . 'pago_proveedor')) {
			...
			$links = "";
			if ($GLOBALS["pago_proveedor_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'pago_proveedor')) {
			...
			if ($GLOBALS["pago_proveedor_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'pago_proveedor')) {
	...
	function SetupOtherOptions() {
		...
		$item->Visible = ($GLOBALS["pago_proveedor"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'pago_proveedor') && $Security->CanAdd());
		if ($item->Visible) {
...
<?php
	...
	if ($proveedor->CurrentAction == "" && $proveedor_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$proveedor_list->setWarningMessage($Language->Phrase("NoPermission"));
