...
$fecha_contable_grid = NULL; // Initialize page object first

class cfecha_contable_grid extends cfecha_contable {
	...
	// Page object name
	var $PageObjName = 'fecha_contable_grid';

	// Grid form hidden field names
	var $FormName = 'ffecha_contablegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["fecha_contable"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idfecha_contable');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idfecha_contable->CurrentValue = strval($arKeys[0]); // idfecha_contable
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
