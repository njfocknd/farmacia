...
$detalle_documento_credito = NULL;

//
// Table class for detalle_documento_credito
//
class cdetalle_documento_credito extends cTable {
	var $iddetalle_documento_credito;
	var $iddocumento_credito;
	var $idproducto;
	var $idbodega;
	var $cantidad;
	var $precio;
	...
	function __construct() {
		...
		$this->TableVar = 'detalle_documento_credito';
		$this->TableName = 'detalle_documento_credito';
		...
		$this->iddetalle_documento_credito = new cField('detalle_documento_credito', 'detalle_documento_credito', 'x_iddetalle_documento_credito', 'iddetalle_documento_credito', '`iddetalle_documento_credito`', '`iddetalle_documento_credito`', 3, -1, FALSE, '`iddetalle_documento_credito`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->iddetalle_documento_credito->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['iddetalle_documento_credito'] = &$this->iddetalle_documento_credito;

		// iddocumento_credito
		$this->iddocumento_credito = new cField('detalle_documento_credito', 'detalle_documento_credito', 'x_iddocumento_credito', 'iddocumento_credito', '`iddocumento_credito`', '`iddocumento_credito`', 3, -1, FALSE, '`iddocumento_credito`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->iddocumento_credito->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['iddocumento_credito'] = &$this->iddocumento_credito;

		// idproducto
		$this->idproducto = new cField('detalle_documento_credito', 'detalle_documento_credito', 'x_idproducto', 'idproducto', '`idproducto`', '`idproducto`', 3, -1, FALSE, '`idproducto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idproducto->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idproducto'] = &$this->idproducto;

		// idbodega
		$this->idbodega = new cField('detalle_documento_credito', 'detalle_documento_credito', 'x_idbodega', 'idbodega', '`idbodega`', '`idbodega`', 3, -1, FALSE, '`idbodega`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idbodega->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idbodega'] = &$this->idbodega;

		// cantidad
		$this->cantidad = new cField('detalle_documento_credito', 'detalle_documento_credito', 'x_cantidad', 'cantidad', '`cantidad`', '`cantidad`', 3, -1, FALSE, '`cantidad`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cantidad->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cantidad'] = &$this->cantidad;

		// precio
		$this->precio = new cField('detalle_documento_credito', 'detalle_documento_credito', 'x_precio', 'precio', '`precio`', '`precio`', 131, -1, FALSE, '`precio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->precio->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['precio'] = &$this->precio;

		// monto
		$this->monto = new cField('detalle_documento_credito', 'detalle_documento_credito', 'x_monto', 'monto', '`monto`', '`monto`', 131, -1, FALSE, '`monto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->monto->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['monto'] = &$this->monto;

		// estado
		$this->estado = new cField('detalle_documento_credito', 'detalle_documento_credito', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "documento_credito") {
			if ($this->iddocumento_credito->getSessionValue() <> "")
				$sMasterFilter .= "`iddocumento_credito`=" . ew_QuotedValue($this->iddocumento_credito->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "documento_credito") {
			if ($this->iddocumento_credito->getSessionValue() <> "")
				$sDetailFilter .= "`iddocumento_credito`=" . ew_QuotedValue($this->iddocumento_credito->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_documento_credito() {
		return "`iddocumento_credito`=@iddocumento_credito@";
	}

	// Detail filter
	function SqlDetailFilter_documento_credito() {
		return "`iddocumento_credito`=@iddocumento_credito@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`detalle_documento_credito`";
	}
	...
	// Update Table
	var $UpdateTable = "`detalle_documento_credito`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('iddetalle_documento_credito', $rs))
				ew_AddFilter($where, ew_QuotedName('iddetalle_documento_credito') . '=' . ew_QuotedValue($rs['iddetalle_documento_credito'], $this->iddetalle_documento_credito->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`iddetalle_documento_credito` = @iddetalle_documento_credito@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->iddetalle_documento_credito->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@iddetalle_documento_credito@", ew_AdjustSql($this->iddetalle_documento_credito->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "detalle_documento_creditolist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("detalle_documento_creditoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("detalle_documento_creditoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "detalle_documento_creditoadd.php?" . $this->UrlParm($parm);
		else
			return "detalle_documento_creditoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("detalle_documento_creditoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("detalle_documento_creditoadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("detalle_documento_creditodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->iddetalle_documento_credito->CurrentValue)) {
			$sUrl .= "iddetalle_documento_credito=" . urlencode($this->iddetalle_documento_credito->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["iddetalle_documento_credito"]; // iddetalle_documento_credito
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->iddetalle_documento_credito->CurrentValue = $key;
   ...
   // Common render codes
		...
		$this->iddetalle_documento_credito->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// iddetalle_documento_credito
		$this->iddetalle_documento_credito->EditAttrs["class"] = "form-control";
		$this->iddetalle_documento_credito->EditCustomAttributes = "";
		$this->iddetalle_documento_credito->EditValue = $this->iddetalle_documento_credito->CurrentValue;
		$this->iddetalle_documento_credito->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->iddocumento_credito->Exportable) $Doc->ExportCaption($this->iddocumento_credito);
					...
					if ($this->iddetalle_documento_credito->Exportable) $Doc->ExportCaption($this->iddetalle_documento_credito);
					if ($this->iddocumento_credito->Exportable) $Doc->ExportCaption($this->iddocumento_credito);
					if ($this->idproducto->Exportable) $Doc->ExportCaption($this->idproducto);
					if ($this->idbodega->Exportable) $Doc->ExportCaption($this->idbodega);
					if ($this->cantidad->Exportable) $Doc->ExportCaption($this->cantidad);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->iddocumento_credito->Exportable) $Doc->ExportField($this->iddocumento_credito);
						...
						if ($this->iddetalle_documento_credito->Exportable) $Doc->ExportField($this->iddetalle_documento_credito);
						if ($this->iddocumento_credito->Exportable) $Doc->ExportField($this->iddocumento_credito);
						if ($this->idproducto->Exportable) $Doc->ExportField($this->idproducto);
						if ($this->idbodega->Exportable) $Doc->ExportField($this->idbodega);
						if ($this->cantidad->Exportable) $Doc->ExportField($this->cantidad);
