...
$meta_grid = NULL; // Initialize page object first

class cmeta_grid extends cmeta {
	...
	// Page object name
	var $PageObjName = 'meta_grid';

	// Grid form hidden field names
	var $FormName = 'fmetagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["meta"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idmeta->CurrentValue = strval($arKeys[0]); // idmeta
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "periodo_contable") {
			$this->idperiodo_contable->Visible = FALSE;
