...
$detalle_documento_debito_grid = NULL; // Initialize page object first

class cdetalle_documento_debito_grid extends cdetalle_documento_debito {
	...
	// Page object name
	var $PageObjName = 'detalle_documento_debito_grid';

	// Grid form hidden field names
	var $FormName = 'fdetalle_documento_debitogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detalle_documento_debito"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('iddetalle_documento_debito');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddetalle_documento_debito->CurrentValue = strval($arKeys[0]); // iddetalle_documento_debito
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "documento_debito") {
			$this->iddocumento_debito->Visible = FALSE;
			if ($GLOBALS["documento_debito"]->EventCancelled) $this->EventCancelled = TRUE;
		}
