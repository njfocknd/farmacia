...
$voucher_tarjeta_list = NULL; // Initialize page object first

class cvoucher_tarjeta_list extends cvoucher_tarjeta {
	...
	// Page object name
	var $PageObjName = 'voucher_tarjeta_list';

	// Grid form hidden field names
	var $FormName = 'fvoucher_tarjetalist';
	...
	function __construct() {
		...
		$this->AddUrl = "voucher_tarjetaadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "voucher_tarjetadelete.php";
		$this->MultiUpdateUrl = "voucher_tarjetaupdate.php";
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idvoucher_tarjeta->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idvoucher_tarjeta->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->ultimos_cuatro_digitos); // ultimos_cuatro_digitos
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->marca->setSort("");
				$this->nombre->setSort("");
				$this->ultimos_cuatro_digitos->setSort("");
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList(CurrentProjectID() . 'pago_cliente')) {
			...
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("pago_clientelist.php?" . EW_TABLE_SHOW_MASTER . "=voucher_tarjeta&fk_idvoucher_tarjeta=" . strval($this->idvoucher_tarjeta->CurrentValue) . "") . "\">" . $body . "</a>";
			$links = "";
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->idvoucher_tarjeta->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'>";
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fvoucher_tarjetalist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fvoucher_tarjetalistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_voucher_tarjeta\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_voucher_tarjeta',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fvoucher_tarjetalist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($voucher_tarjeta_list)) $voucher_tarjeta_list = new cvoucher_tarjeta_list();

// Page init
$voucher_tarjeta_list->Page_Init();

// Page main
$voucher_tarjeta_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$voucher_tarjeta_list->Page_Render();
?>
...
// Page object
var voucher_tarjeta_list = new ew_Page("voucher_tarjeta_list");
voucher_tarjeta_list.PageID = "list"; // Page ID
var EW_PAGE_ID = voucher_tarjeta_list.PageID; // For backward compatibility

// Form object
var fvoucher_tarjetalist = new ew_Form("fvoucher_tarjetalist");
fvoucher_tarjetalist.FormKeyCountName = '<?php echo $voucher_tarjeta_list->FormKeyCountName ?>';

// Form_CustomValidate event
fvoucher_tarjetalist.Form_CustomValidate = 
...
fvoucher_tarjetalist.ValidateRequired = true;
<?php } else { ?>
fvoucher_tarjetalist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fvoucher_tarjetalist.Lists["x_idbanco"] = {"LinkField":"x_idbanco","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fvoucher_tarjetalist.Lists["x_idcuenta"] = {"LinkField":"x_idcuenta","Ajax":true,"AutoFill":false,"DisplayFields":["x_numero","x_nombre","",""],"ParentFields":[],"FilterFields":[],"Options":[]};

// Form object for search
var fvoucher_tarjetalistsrch = new ew_Form("fvoucher_tarjetalistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($voucher_tarjeta_list->TotalRecs > 0 && $voucher_tarjeta->getCurrentMasterTable() == "" && $voucher_tarjeta_list->ExportOptions->Visible()) { ?>
<?php $voucher_tarjeta_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($voucher_tarjeta_list->SearchOptions->Visible()) { ?>
<?php $voucher_tarjeta_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($voucher_tarjeta->Export == "") { ?>
...
<?php } ?>
<?php if (($voucher_tarjeta->Export == "") || (EW_EXPORT_MASTER_RECORD && $voucher_tarjeta->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "cuentalist.php";
if ($voucher_tarjeta_list->DbMasterFilter <> "" && $voucher_tarjeta->getCurrentMasterTable() == "cuenta") {
	if ($voucher_tarjeta_list->MasterRecordExists) {
		if ($voucher_tarjeta->getCurrentMasterTable() == $voucher_tarjeta->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($voucher_tarjeta_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $voucher_tarjeta_list->ExportOptions->Render("body") ?></div>
<?php } ?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$voucher_tarjeta_list->TotalRecs = $voucher_tarjeta->SelectRecordCount();
	} else {
		if ($voucher_tarjeta_list->Recordset = $voucher_tarjeta_list->LoadRecordset())
			$voucher_tarjeta_list->TotalRecs = $voucher_tarjeta_list->Recordset->RecordCount();
	}
	$voucher_tarjeta_list->StartRec = 1;
	if ($voucher_tarjeta_list->DisplayRecs <= 0 || ($voucher_tarjeta->Export <> "" && $voucher_tarjeta->ExportAll)) // Display all records
		$voucher_tarjeta_list->DisplayRecs = $voucher_tarjeta_list->TotalRecs;
	if (!($voucher_tarjeta->Export <> "" && $voucher_tarjeta->ExportAll))
		$voucher_tarjeta_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$voucher_tarjeta_list->Recordset = $voucher_tarjeta_list->LoadRecordset($voucher_tarjeta_list->StartRec-1, $voucher_tarjeta_list->DisplayRecs);

	// Set no record found message
	if ($voucher_tarjeta->CurrentAction == "" && $voucher_tarjeta_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$voucher_tarjeta_list->setWarningMessage($Language->Phrase("NoPermission"));
		if ($voucher_tarjeta_list->SearchWhere == "0=101")
			$voucher_tarjeta_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$voucher_tarjeta_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$voucher_tarjeta_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($voucher_tarjeta->Export == "" && $voucher_tarjeta->CurrentAction == "") { ?>
<form name="fvoucher_tarjetalistsrch" id="fvoucher_tarjetalistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($voucher_tarjeta_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fvoucher_tarjetalistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="voucher_tarjeta">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($voucher_tarjeta_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($voucher_tarjeta_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $voucher_tarjeta_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($voucher_tarjeta_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($voucher_tarjeta_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($voucher_tarjeta_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($voucher_tarjeta_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $voucher_tarjeta_list->ShowPageHeader(); ?>
<?php
$voucher_tarjeta_list->ShowMessage();
?>
<?php if ($voucher_tarjeta_list->TotalRecs > 0 || $voucher_tarjeta->CurrentAction <> "") { ?>
<div class="ewGrid">
<form name="fvoucher_tarjetalist" id="fvoucher_tarjetalist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($voucher_tarjeta_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $voucher_tarjeta_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="voucher_tarjeta">
<div id="gmp_voucher_tarjeta" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($voucher_tarjeta_list->TotalRecs > 0) { ?>
<table id="tbl_voucher_tarjetalist" class="table ewTable">
...
// Render list options
$voucher_tarjeta_list->RenderListOptions();

// Render list options (header, left)
$voucher_tarjeta_list->ListOptions->Render("header", "left");
?>
<?php if ($voucher_tarjeta->idbanco->Visible) { // idbanco ?>
	...
	<?php } else { ?>
		<th data-name="idbanco"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $voucher_tarjeta->SortUrl($voucher_tarjeta->idbanco) ?>',1);"><div id="elh_voucher_tarjeta_idbanco" class="voucher_tarjeta_idbanco">
...
<?php if ($voucher_tarjeta->idcuenta->Visible) { // idcuenta ?>
	...
	<?php } else { ?>
		<th data-name="idcuenta"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $voucher_tarjeta->SortUrl($voucher_tarjeta->idcuenta) ?>',1);"><div id="elh_voucher_tarjeta_idcuenta" class="voucher_tarjeta_idcuenta">
...
<?php if ($voucher_tarjeta->marca->Visible) { // marca ?>
	...
	<?php } else { ?>
		<th data-name="marca"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $voucher_tarjeta->SortUrl($voucher_tarjeta->marca) ?>',1);"><div id="elh_voucher_tarjeta_marca" class="voucher_tarjeta_marca">
...
<?php if ($voucher_tarjeta->nombre->Visible) { // nombre ?>
	...
	<?php } else { ?>
		<th data-name="nombre"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $voucher_tarjeta->SortUrl($voucher_tarjeta->nombre) ?>',1);"><div id="elh_voucher_tarjeta_nombre" class="voucher_tarjeta_nombre">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $voucher_tarjeta->nombre->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($voucher_tarjeta->nombre->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($voucher_tarjeta->nombre->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($voucher_tarjeta->ultimos_cuatro_digitos->Visible) { // ultimos_cuatro_digitos ?>
	...
	<?php } else { ?>
		<th data-name="ultimos_cuatro_digitos"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $voucher_tarjeta->SortUrl($voucher_tarjeta->ultimos_cuatro_digitos) ?>',1);"><div id="elh_voucher_tarjeta_ultimos_cuatro_digitos" class="voucher_tarjeta_ultimos_cuatro_digitos">
...
<?php if ($voucher_tarjeta->referencia->Visible) { // referencia ?>
	...
	<?php } else { ?>
		<th data-name="referencia"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $voucher_tarjeta->SortUrl($voucher_tarjeta->referencia) ?>',1);"><div id="elh_voucher_tarjeta_referencia" class="voucher_tarjeta_referencia">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $voucher_tarjeta->referencia->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($voucher_tarjeta->referencia->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($voucher_tarjeta->referencia->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($voucher_tarjeta->fecha->Visible) { // fecha ?>
	...
	<?php } else { ?>
		<th data-name="fecha"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $voucher_tarjeta->SortUrl($voucher_tarjeta->fecha) ?>',1);"><div id="elh_voucher_tarjeta_fecha" class="voucher_tarjeta_fecha">
...
<?php if ($voucher_tarjeta->monto->Visible) { // monto ?>
	...
	<?php } else { ?>
		<th data-name="monto"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $voucher_tarjeta->SortUrl($voucher_tarjeta->monto) ?>',1);"><div id="elh_voucher_tarjeta_monto" class="voucher_tarjeta_monto">
...
$voucher_tarjeta_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($voucher_tarjeta->ExportAll && $voucher_tarjeta->Export <> "") {
	$voucher_tarjeta_list->StopRec = $voucher_tarjeta_list->TotalRecs;
} else {

	// Set the last record to display
	if ($voucher_tarjeta_list->TotalRecs > $voucher_tarjeta_list->StartRec + $voucher_tarjeta_list->DisplayRecs - 1)
		$voucher_tarjeta_list->StopRec = $voucher_tarjeta_list->StartRec + $voucher_tarjeta_list->DisplayRecs - 1;
	else
		$voucher_tarjeta_list->StopRec = $voucher_tarjeta_list->TotalRecs;
}
$voucher_tarjeta_list->RecCnt = $voucher_tarjeta_list->StartRec - 1;
if ($voucher_tarjeta_list->Recordset && !$voucher_tarjeta_list->Recordset->EOF) {
	$voucher_tarjeta_list->Recordset->MoveFirst();
	$bSelectLimit = EW_SELECT_LIMIT;
	if (!$bSelectLimit && $voucher_tarjeta_list->StartRec > 1)
		$voucher_tarjeta_list->Recordset->Move($voucher_tarjeta_list->StartRec - 1);
} elseif (!$voucher_tarjeta->AllowAddDeleteRow && $voucher_tarjeta_list->StopRec == 0) {
	$voucher_tarjeta_list->StopRec = $voucher_tarjeta->GridAddRowCount;
}

// Initialize aggregate
$voucher_tarjeta->RowType = EW_ROWTYPE_AGGREGATEINIT;
$voucher_tarjeta->ResetAttrs();
$voucher_tarjeta_list->RenderRow();
while ($voucher_tarjeta_list->RecCnt < $voucher_tarjeta_list->StopRec) {
	$voucher_tarjeta_list->RecCnt++;
	if (intval($voucher_tarjeta_list->RecCnt) >= intval($voucher_tarjeta_list->StartRec)) {
		$voucher_tarjeta_list->RowCnt++;

		// Set up key count
		$voucher_tarjeta_list->KeyCount = $voucher_tarjeta_list->RowIndex;
		...
		if ($voucher_tarjeta->CurrentAction == "gridadd") {
			...
			$voucher_tarjeta_list->LoadRowValues($voucher_tarjeta_list->Recordset); // Load row values
		}
		$voucher_tarjeta->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$voucher_tarjeta->RowAttrs = array_merge($voucher_tarjeta->RowAttrs, array('data-rowindex'=>$voucher_tarjeta_list->RowCnt, 'id'=>'r' . $voucher_tarjeta_list->RowCnt . '_voucher_tarjeta', 'data-rowtype'=>$voucher_tarjeta->RowType));
...
$voucher_tarjeta_list->ListOptions->Render("body", "left", $voucher_tarjeta_list->RowCnt);
?>
	<?php if ($voucher_tarjeta->idbanco->Visible) { // idbanco ?>
...
<a id="<?php echo $voucher_tarjeta_list->PageObjName . "_row_" . $voucher_tarjeta_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($voucher_tarjeta->idcuenta->Visible) { // idcuenta ?>
...
$voucher_tarjeta_list->ListOptions->Render("body", "right", $voucher_tarjeta_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($voucher_tarjeta->CurrentAction <> "gridadd")
		$voucher_tarjeta_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($voucher_tarjeta->CurrentAction == "") { ?>
...
// Close recordset
if ($voucher_tarjeta_list->Recordset)
	$voucher_tarjeta_list->Recordset->Close();
?>
<?php if ($voucher_tarjeta->Export == "") { ?>
<div class="ewGridLowerPanel">
<?php if ($voucher_tarjeta->CurrentAction <> "gridadd" && $voucher_tarjeta->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($voucher_tarjeta_list->Pager)) $voucher_tarjeta_list->Pager = new cPrevNextPager($voucher_tarjeta_list->StartRec, $voucher_tarjeta_list->DisplayRecs, $voucher_tarjeta_list->TotalRecs) ?>
<?php if ($voucher_tarjeta_list->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($voucher_tarjeta_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $voucher_tarjeta_list->PageUrl() ?>start=<?php echo $voucher_tarjeta_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($voucher_tarjeta_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $voucher_tarjeta_list->PageUrl() ?>start=<?php echo $voucher_tarjeta_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $voucher_tarjeta_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($voucher_tarjeta_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $voucher_tarjeta_list->PageUrl() ?>start=<?php echo $voucher_tarjeta_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($voucher_tarjeta_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $voucher_tarjeta_list->PageUrl() ?>start=<?php echo $voucher_tarjeta_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $voucher_tarjeta_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $voucher_tarjeta_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $voucher_tarjeta_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $voucher_tarjeta_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($voucher_tarjeta_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($voucher_tarjeta_list->TotalRecs == 0 && $voucher_tarjeta->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($voucher_tarjeta_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($voucher_tarjeta->Export == "") { ?>
<script type="text/javascript">
fvoucher_tarjetalistsrch.Init();
fvoucher_tarjetalist.Init();
</script>
<?php } ?>
<?php
$voucher_tarjeta_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($voucher_tarjeta->Export == "") { ?>
...
<?php
$voucher_tarjeta_list->Page_Terminate();
?>
