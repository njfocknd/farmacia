...
$documento_grid = NULL; // Initialize page object first

class cdocumento_grid extends cdocumento {
	...
	// Page object name
	var $PageObjName = 'documento_grid';

	// Grid form hidden field names
	var $FormName = 'fdocumentogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["documento"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->iddocumento->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->iddocumento->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->iddocumento->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddocumento->CurrentValue = strval($arKeys[0]); // iddocumento
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
