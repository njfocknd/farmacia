...
<?php include_once $EW_RELATIVE_PATH . "audittrailinfo.php" ?>
...
class caudittrail_search extends caudittrail {
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->user->CurrentValue) <> "") {
				$sFilterWrk = "`idusuario`" . ew_SearchString("=", $this->user->CurrentValue, EW_DATATYPE_NUMBER);
			$sSqlWrk = "SELECT `idusuario`, `usuario` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `usuario`";
			$sWhereWrk = "";
			if ($sFilterWrk <> "") {
			...
			$this->Lookup_Selecting($this->user, $sWhereWrk);
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->user->ViewValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
			...
			$this->datetime->EditValue = ew_HtmlEncode(ew_FormatDateTime(ew_UnFormatDateTime($this->datetime->AdvancedSearch->SearchValue, 7), 7));
			...
			if (trim(strval($this->user->AdvancedSearch->SearchValue)) == "") {
				...
				$sFilterWrk = "`idusuario`" . ew_SearchString("=", $this->user->AdvancedSearch->SearchValue, EW_DATATYPE_NUMBER);
			}
			$sSqlWrk = "SELECT `idusuario`, `usuario` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `usuario`";
			$sWhereWrk = "";
			if ($sFilterWrk <> "") {
				ew_AddFilter($sWhereWrk, $sFilterWrk);
			}
			if (!$GLOBALS["audittrail"]->UserIDAllow("search")) $sWhereWrk = $GLOBALS["usuario"]->AddUserIDFilter($sWhereWrk);
	...
	function ValidateSearch() {
		...
		if (!ew_CheckEuroDate($this->datetime->AdvancedSearch->SearchValue)) {
...
faudittrailsearch.Lists["x_user"] = {"LinkField":"x_idusuario","Ajax":true,"AutoFill":false,"DisplayFields":["x_usuario","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="faudittrailsearch" id="faudittrailsearch" class="form-horizontal ewForm ewSearchForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="text" data-field="x_id" name="x_id" id="x_id" placeholder="<?php echo ew_HtmlEncode($audittrail->id->PlaceHolder) ?>" value="<?php echo $audittrail->id->EditValue ?>"<?php echo $audittrail->id->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
...
<select data-field="x_user" id="x_user" name="x_user"<?php echo $audittrail->user->EditAttributes() ?>>
<?php
if (is_array($audittrail->user->EditValue)) {
	$arwrk = $audittrail->user->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($audittrail->user->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
// Call Lookup selecting
$audittrail->Lookup_Selecting($audittrail->user, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
<input type="hidden" name="s_x_user" id="s_x_user" value="s=<?php echo ew_Encrypt($sSqlWrk) ?>&amp;f0=<?php echo ew_Encrypt("`idusuario` = {filter_value}"); ?>&amp;t0=3">
</span>
		</div></div>
	</div>
<?php } ?>
...
<input type="text" data-field="x_keyvalue" name="x_keyvalue" id="x_keyvalue" size="35" placeholder="<?php echo ew_HtmlEncode($audittrail->keyvalue->PlaceHolder) ?>" value="<?php echo $audittrail->keyvalue->EditValue ?>"<?php echo $audittrail->keyvalue->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
...
<input type="text" data-field="x_oldvalue" name="x_oldvalue" id="x_oldvalue" size="35" placeholder="<?php echo ew_HtmlEncode($audittrail->oldvalue->PlaceHolder) ?>" value="<?php echo $audittrail->oldvalue->EditValue ?>"<?php echo $audittrail->oldvalue->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
...
<input type="text" data-field="x_newvalue" name="x_newvalue" id="x_newvalue" size="35" placeholder="<?php echo ew_HtmlEncode($audittrail->newvalue->PlaceHolder) ?>" value="<?php echo $audittrail->newvalue->EditValue ?>"<?php echo $audittrail->newvalue->EditAttributes() ?>>
</span>
		</div></div>
	</div>
<?php } ?>
