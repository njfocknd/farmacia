...
class cpais_list extends cpais {
	...
	function SetupListOptions() {
		...
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'departamento') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
		...
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'moneda') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
		$item->ShowInButtonGroup = FALSE;
		if (!isset($GLOBALS["moneda_grid"])) $GLOBALS["moneda_grid"] = new cmoneda_grid;
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList(CurrentProjectID() . 'departamento')) {
			...
			$links = "";
			if ($GLOBALS["departamento_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'departamento')) {
			...
			if ($GLOBALS["departamento_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'departamento')) {
		...
		if ($Security->AllowList(CurrentProjectID() . 'moneda')) {
			...
			$links = "";
			if ($GLOBALS["moneda_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'moneda')) {
			...
			if ($GLOBALS["moneda_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'moneda')) {
	...
	function SetupOtherOptions() {
		...
		$item->Visible = ($GLOBALS["departamento"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'departamento') && $Security->CanAdd());
		if ($item->Visible) {
		...
		$item->Visible = ($GLOBALS["moneda"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'moneda') && $Security->CanAdd());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
...
<?php
	...
	if ($pais->CurrentAction == "" && $pais_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$pais_list->setWarningMessage($Language->Phrase("NoPermission"));
