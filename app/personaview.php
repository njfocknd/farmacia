...
class cpersona_view extends cpersona {
	...
	function SetupOtherOptions() {
		...
		$links = "";
		if ($GLOBALS["cliente_grid"] && $GLOBALS["cliente_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'cliente')) {
		...
		if ($GLOBALS["cliente_grid"] && $GLOBALS["cliente_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'cliente')) {
		...
		if ($GLOBALS["cliente_grid"] && $GLOBALS["cliente_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'cliente')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'proveedor');
		if ($item->Visible) {
		...
		$links = "";
		if ($GLOBALS["proveedor_grid"] && $GLOBALS["proveedor_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'proveedor')) {
		...
		if ($GLOBALS["proveedor_grid"] && $GLOBALS["proveedor_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'proveedor')) {
		...
		if ($GLOBALS["proveedor_grid"] && $GLOBALS["proveedor_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'proveedor')) {
	...
	function ExportData() {
		...
		// Export detail records (cliente)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("cliente", explode(",", $this->getCurrentDetailTable()))) {
			global $cliente;
			if (!isset($cliente)) $cliente = new ccliente;
			$rsdetail = $cliente->LoadRs($cliente->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$cliente->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
		...
		// Export detail records (proveedor)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("proveedor", explode(",", $this->getCurrentDetailTable()))) {
			global $proveedor;
			if (!isset($proveedor)) $proveedor = new cproveedor;
			$rsdetail = $proveedor->LoadRs($proveedor->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("h"); // Change to horizontal
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					$Doc->ExportEmptyRow();
					$detailcnt = $rsdetail->RecordCount();
					$proveedor->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
				$Doc->SetStyle($ExportStyle); // Restore
				$rsdetail->Close();
			}
