...
$detalle_documento_add = NULL; // Initialize page object first

class cdetalle_documento_add extends cdetalle_documento {
	...
	// Page object name
	var $PageObjName = 'detalle_documento_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["iddetalle_documento"] != "") {
				...
				$this->setKey("iddetalle_documento", $this->iddetalle_documento->CurrentValue); // Set up key
			} else {
				$this->setKey("iddetalle_documento", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "detalle_documentoview.php")
	...
	function LoadDefaultValues() {
		$this->iddocumento->CurrentValue = 1;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->iddocumento->getSessionValue() <> "") {
				$this->iddocumento->CurrentValue = $this->iddocumento->getSessionValue();
			if (strval($this->iddocumento->CurrentValue) <> "") {
				$sFilterWrk = "`iddocumento`" . ew_SearchString("=", $this->iddocumento->CurrentValue, EW_DATATYPE_NUMBER);
			$sSqlWrk = "SELECT `iddocumento`, `serie` AS `DispFld`, '' AS `Disp2Fld`, `correlativo` AS `Disp3Fld`, '' AS `Disp4Fld` FROM `documento`";
			$sWhereWrk = "";
			...
			if (trim(strval($this->iddocumento->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`iddocumento`" . ew_SearchString("=", $this->iddocumento->CurrentValue, EW_DATATYPE_NUMBER);
			}
			$sSqlWrk = "SELECT `iddocumento`, `serie` AS `DispFld`, '' AS `Disp2Fld`, `correlativo` AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `documento`";
			$sWhereWrk = "";
			$lookuptblfilter = "`estado` = 'Activo'";
			if (strval($lookuptblfilter) <> "") {
				ew_AddFilter($sWhereWrk, $lookuptblfilter);
			...
			$this->iddocumento->EditValue = $arwrk;
			}
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!$this->iddocumento->FldIsDetailKey && !is_null($this->iddocumento->FormValue) && $this->iddocumento->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->iddocumento->FldCaption(), $this->iddocumento->ReqErrMsg));
		}
	...
	function AddRow($rsold = NULL) {
		...
		// iddocumento
		$this->iddocumento->SetDbValueDef($rsnew, $this->iddocumento->CurrentValue, 0, strval($this->iddocumento->CurrentValue) == "");
		...
		if ($AddRow) {
			$this->iddetalle_documento->setDbValue($conn->Insert_ID());
			$rsnew['iddetalle_documento'] = $this->iddetalle_documento->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($detalle_documento_add)) $detalle_documento_add = new cdetalle_documento_add();

// Page init
$detalle_documento_add->Page_Init();

// Page main
$detalle_documento_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detalle_documento_add->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var detalle_documento_add = new ew_Page("detalle_documento_add");
detalle_documento_add.PageID = "add"; // Page ID
var EW_PAGE_ID = detalle_documento_add.PageID; // For backward compatibility

// Form object
var fdetalle_documentoadd = new ew_Form("fdetalle_documentoadd");

// Validate form
fdetalle_documentoadd.Validate = function() {
...
fdetalle_documentoadd.Form_CustomValidate = 
...
fdetalle_documentoadd.ValidateRequired = true;
<?php } else { ?>
fdetalle_documentoadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdetalle_documentoadd.Lists["x_iddocumento"] = {"LinkField":"x_iddocumento","Ajax":true,"AutoFill":false,"DisplayFields":["x_serie","","x_correlativo",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fdetalle_documentoadd.Lists["x_idproducto"] = {"LinkField":"x_idproducto","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fdetalle_documentoadd.Lists["x_idbodega"] = {"LinkField":"x_idbodega","Ajax":true,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $detalle_documento_add->ShowPageHeader(); ?>
<?php
$detalle_documento_add->ShowMessage();
?>
<form name="fdetalle_documentoadd" id="fdetalle_documentoadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($detalle_documento_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $detalle_documento_add->Token ?>">
<?php } ?>
...
<?php if ($detalle_documento->iddocumento->Visible) { // iddocumento ?>
	<div id="r_iddocumento" class="form-group">
		<label id="elh_detalle_documento_iddocumento" for="x_iddocumento" class="col-sm-2 control-label ewLabel"><?php echo $detalle_documento->iddocumento->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $detalle_documento->iddocumento->CellAttributes() ?>>
<?php if ($detalle_documento->iddocumento->getSessionValue() <> "") { ?>
<span id="el_detalle_documento_iddocumento">
...
<input type="hidden" id="x_iddocumento" name="x_iddocumento" value="<?php echo ew_HtmlEncode($detalle_documento->iddocumento->CurrentValue) ?>">
<?php } else { ?>
<span id="el_detalle_documento_iddocumento">
<select data-field="x_iddocumento" id="x_iddocumento" name="x_iddocumento"<?php echo $detalle_documento->iddocumento->EditAttributes() ?>>
<?php
...
$sSqlWrk .= " ORDER BY `correlativo`";
?>
<input type="hidden" name="s_x_iddocumento" id="s_x_iddocumento" value="s=<?php echo ew_Encrypt($sSqlWrk) ?>&amp;f0=<?php echo ew_Encrypt("`iddocumento` = {filter_value}"); ?>&amp;t0=3">
</span>
<?php } ?>
<?php echo $detalle_documento->iddocumento->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($detalle_documento->idproducto->Visible) { // idproducto ?>
	<div id="r_idproducto" class="form-group">
		<label id="elh_detalle_documento_idproducto" for="x_idproducto" class="col-sm-2 control-label ewLabel"><?php echo $detalle_documento->idproducto->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $detalle_documento->idproducto->CellAttributes() ?>>
<span id="el_detalle_documento_idproducto">
<select data-field="x_idproducto" id="x_idproducto" name="x_idproducto"<?php echo $detalle_documento->idproducto->EditAttributes() ?>>
<?php
...
<?php echo $detalle_documento->idproducto->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($detalle_documento->idbodega->Visible) { // idbodega ?>
	<div id="r_idbodega" class="form-group">
		<label id="elh_detalle_documento_idbodega" for="x_idbodega" class="col-sm-2 control-label ewLabel"><?php echo $detalle_documento->idbodega->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $detalle_documento->idbodega->CellAttributes() ?>>
<span id="el_detalle_documento_idbodega">
<select data-field="x_idbodega" id="x_idbodega" name="x_idbodega"<?php echo $detalle_documento->idbodega->EditAttributes() ?>>
<?php
...
<?php echo $detalle_documento->idbodega->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
fdetalle_documentoadd.Init();
</script>
<?php
$detalle_documento_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$detalle_documento_add->Page_Terminate();
?>
