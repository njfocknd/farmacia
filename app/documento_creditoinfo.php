...
$documento_credito = NULL;

//
// Table class for documento_credito
//
class cdocumento_credito extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'documento_credito';
		$this->TableName = 'documento_credito';
		...
		// iddocumento_credito
		$this->iddocumento_credito = new cField('documento_credito', 'documento_credito', 'x_iddocumento_credito', 'iddocumento_credito', '`iddocumento_credito`', '`iddocumento_credito`', 3, -1, FALSE, '`iddocumento_credito`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// idtipo_documento
		$this->idtipo_documento = new cField('documento_credito', 'documento_credito', 'x_idtipo_documento', 'idtipo_documento', '`idtipo_documento`', '`idtipo_documento`', 3, -1, FALSE, '`idtipo_documento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// idsucursal
		$this->idsucursal = new cField('documento_credito', 'documento_credito', 'x_idsucursal', 'idsucursal', '`idsucursal`', '`idsucursal`', 3, -1, FALSE, '`idsucursal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idsucursal->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idsucursal'] = &$this->idsucursal;

		// serie
		$this->serie = new cField('documento_credito', 'documento_credito', 'x_serie', 'serie', '`serie`', '`serie`', 200, -1, FALSE, '`serie`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['serie'] = &$this->serie;

		// correlativo
		$this->correlativo = new cField('documento_credito', 'documento_credito', 'x_correlativo', 'correlativo', '`correlativo`', '`correlativo`', 3, -1, FALSE, '`correlativo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->correlativo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['correlativo'] = &$this->correlativo;

		// fecha
		$this->fecha = new cField('documento_credito', 'documento_credito', 'x_fecha', 'fecha', '`fecha`', 'DATE_FORMAT(`fecha`, \'%d/%m/%Y\')', 133, 7, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha'] = &$this->fecha;

		// observaciones
		$this->observaciones = new cField('documento_credito', 'documento_credito', 'x_observaciones', 'observaciones', '`observaciones`', '`observaciones`', 200, -1, FALSE, '`observaciones`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['observaciones'] = &$this->observaciones;

		// estado_documento
		$this->estado_documento = new cField('documento_credito', 'documento_credito', 'x_estado_documento', 'estado_documento', '`estado_documento`', '`estado_documento`', 202, -1, FALSE, '`estado_documento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['estado_documento'] = &$this->estado_documento;

		// estado
		$this->estado = new cField('documento_credito', 'documento_credito', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['estado'] = &$this->estado;

		// monto
		$this->monto = new cField('documento_credito', 'documento_credito', 'x_monto', 'monto', '`monto`', '`monto`', 131, -1, FALSE, '`monto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->monto->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['monto'] = &$this->monto;

		// fecha_insercion
		$this->fecha_insercion = new cField('documento_credito', 'documento_credito', 'x_fecha_insercion', 'fecha_insercion', '`fecha_insercion`', 'DATE_FORMAT(`fecha_insercion`, \'%d/%m/%Y\')', 133, 7, FALSE, '`fecha_insercion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// idproveedor
		$this->idproveedor = new cField('documento_credito', 'documento_credito', 'x_idproveedor', 'idproveedor', '`idproveedor`', '`idproveedor`', 3, -1, FALSE, '`idproveedor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "detalle_documento_credito") {
			$sDetailUrl = $GLOBALS["detalle_documento_credito"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&fk_iddocumento_credito=" . urlencode($this->iddocumento_credito->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "documento_creditolist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`documento_credito`";
	}
	...
	// Update Table
	var $UpdateTable = "`documento_credito`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('iddocumento_credito', $rs))
				ew_AddFilter($where, ew_QuotedName('iddocumento_credito') . '=' . ew_QuotedValue($rs['iddocumento_credito'], $this->iddocumento_credito->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->iddocumento_credito->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@iddocumento_credito@", ew_AdjustSql($this->iddocumento_credito->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "documento_creditolist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("documento_creditoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("documento_creditoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "documento_creditoadd.php?" . $this->UrlParm($parm);
		else
			return "documento_creditoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("documento_creditoedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("documento_creditoedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("documento_creditoadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("documento_creditoadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("documento_creditodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->iddocumento_credito->CurrentValue)) {
			$sUrl .= "iddocumento_credito=" . urlencode($this->iddocumento_credito->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["iddocumento_credito"]; // iddocumento_credito
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->iddocumento_credito->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->iddocumento_credito->EditValue = $this->iddocumento_credito->CurrentValue;
		$this->iddocumento_credito->ViewCustomAttributes = "";
