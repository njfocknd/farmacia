...
$detalle_documento_grid = NULL; // Initialize page object first

class cdetalle_documento_grid extends cdetalle_documento {
	...
	// Page object name
	var $PageObjName = 'detalle_documento_grid';

	// Grid form hidden field names
	var $FormName = 'fdetalle_documentogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detalle_documento"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->iddetalle_documento->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_iddocumento") && $objForm->HasValue("o_iddocumento") && $this->iddocumento->CurrentValue <> $this->iddocumento->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->iddetalle_documento->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('iddetalle_documento');
	}
	...
	function LoadDefaultValues() {
		...
		$this->iddocumento->OldValue = $this->iddocumento->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->iddocumento->setOldValue($objForm->GetValue("o_iddocumento"));
		...
		if (!$this->iddetalle_documento->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddetalle_documento->CurrentValue = strval($arKeys[0]); // iddetalle_documento
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['iddetalle_documento'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// iddocumento
			$this->iddocumento->SetDbValueDef($rsnew, $this->iddocumento->CurrentValue, 0, $this->iddocumento->ReadOnly);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "documento") {
			...
			if ($GLOBALS["documento"]->EventCancelled) $this->EventCancelled = TRUE;
		}
