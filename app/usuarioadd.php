...
class cusuario_add extends cusuario {
	...
	function AddRow($rsold = NULL) {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$bValidParentUser = $Security->IsValidUserID($this->userlevelid->CurrentValue);
			if (!$bValidParentUser) {
				$sParentUserIdMsg = str_replace("%c", CurrentUserID(), $Language->Phrase("UnAuthorizedParentUserID"));
				$sParentUserIdMsg = str_replace("%p", $this->userlevelid->CurrentValue, $sParentUserIdMsg);
		...
		// contrasena
		$this->contrasena->SetDbValueDef($rsnew, $this->contrasena->CurrentValue, "", FALSE);
...
fusuarioadd.Lists["x_userlevelid"] = {"LinkField":"x_userlevelid","Ajax":null,"AutoFill":false,"DisplayFields":["x_userlevelname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fusuarioadd" id="fusuarioadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<script type="text/javascript">
fusuarioadd.Lists["x_userlevelid"].Options = <?php echo (is_array($usuario->userlevelid->EditValue)) ? ew_ArrayToJson($usuario->userlevelid->EditValue, 1) : "[]" ?>;
</script>
</span>
